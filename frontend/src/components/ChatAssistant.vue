...
registerPostProcessor('form', async (msg, intentData) => {
  ...
  // 如果有待确认表单，说明用户正在完成当前表单的流程，不应该拦截
  // 让用户继续与 AI 对话，由 doSendMessage 中的 checkUserConfirmation 处理
  if (hasPendingConfirm) {
    console.log('[form 后处理] 有待确认表单，不拦截，让 doSendMessage 处理')
    msg.content = msg.streamText || ''
    return
  }
...
registerPostProcessor('validate', async (msg) => {
  ...
  console.log('[validate 后处理] validationPass:', !!validationPass, 'validationFail:', !!validationFail)
  console.log('[validate 后处理] pendingConfirmForm:', pendingConfirmForm.value ? '存在' : 'null')
  ...
  if (validationFail) {
    ...
    console.log('[validate 后处理] 校验通过，pendingConfirmForm 状态:', pendingConfirmForm.value ? '存在' : 'null')
    if (pendingConfirmForm.value) {
      ...
      const confirmText = `\n\n---\n\n✅ 校验通过！请确认是否提交？（回复"确认"、"好的"或"提交"执行提交，回复"取消"放弃提交）`
      msg.streamText = (msg.streamText || '') + confirmText
      msg.content = msg.streamText  // 确保 content 也更新，用于保存
      console.log('[validate 后处理] 已添加确认提示')
    } else {
      console.warn('[validate 后处理] 校验通过但 pendingConfirmForm 为空！')
    }
  } else {
...
const doSendMessageAfterHome = async (text, { skipUserPush = false, formCode = null, formData = null } = {}) => {
  ...
  try {
    ...
    if (msg) {
      ...
      console.log('[SSE 流结束] intentType:', intentType, 'intentData:', intentData)
      const postProcessor = getPostProcessor(intentType)
      console.log('[SSE 流结束] postProcessor:', postProcessor ? '存在' : 'null')
      if (postProcessor) {
        console.log('[SSE 流结束] 调用后处理器:', intentType)
