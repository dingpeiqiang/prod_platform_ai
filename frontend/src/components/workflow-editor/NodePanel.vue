...
### 输入参数配置
- **自动生成**：根据工具的 input_schema 自动生成参数输入框
- **参数类型**：支持字符串、数字、布尔值、数组、对象和变量引用
- **变量引用**：选择"变量引用"类型后，可从级联选择器中选择工作流变量
- **默认值**：支持设置默认参数值

### 输出参数配置
- **输出字段**：显示工具 output_schema 中定义的所有输出字段
- **点击选择**：点击输出字段可快速添加到输出映射列表
- **输出映射**：配置工具输出字段到工作流变量的映射关系
  - 源字段：选择工具输出的字段名
  - 目标变量名：指定映射后的工作流变量名
  - 类型：指定输出变量类型（string/number/boolean/object/array）

### 执行配置
- **超时时间**：设置工具执行超时时间（1-600秒）
- **异步执行**：开启后工具将异步执行，不阻塞工作流
- **静默模式**：开启后不输出执行日志

### 使用示例
\`\`\`
1. 从下拉列表选择工具（如 query_form_template）
2. 在"输入参数"区域为每个参数填写值或选择变量引用
3. 在"输出参数"区域点击需要的输出字段或手动添加映射
4. 配置执行选项（超时时间、异步模式等）
5. 执行时参数值会通过变量引用语法替换为实际值
6. 工具执行结果按输出映射配置传递给工作流变量
\`\`\`

### 注意事项
- 工具列表在节点创建时从后端 API 加载
- 如需刷新工具列表，可切换分类或重新打开节点
- 工具执行结果通过输出映射配置传递给工作流变量
  `,
  http: `
## HTTP请求节点
