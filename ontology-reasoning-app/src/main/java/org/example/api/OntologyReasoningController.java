...
public class OntologyReasoningController {
    ...
    @PostMapping("/evaluate") public EvaluatePolicyResponse evaluateWithFacts(@RequestBody Map<String, Object> payload) {
        ...
        var trace = new org.example.model.Models.TraceContext(null, asString(payload.get("tenant_id"), "marketing_tenant"), null);
        var refs = entities.stream().map(item -> new org.example.model.Models.EntityRef(asString(item.get("id"), null), asString(item.get("type"), "Entity"), asString(item.get("source"), "ontology"))).toList();
