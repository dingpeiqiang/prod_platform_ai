...
- chat_sessions: 通用会话表（支持 metadata / context_tags / parent_session）
- chat_messages: 通用消息表（只存核心字段，业务全入 metadata）
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_sessions_id'),          'chat_sessions', ['id'], unique=False)
    op.create_index(op.f('ix_sessions_session_id'),  'chat_sessions', ['session_id'], unique=True)
    op.create_index(op.f('ix_sessions_user_id'),     'chat_sessions', ['user_id'], unique=False)
    op.create_index(op.f('ix_sessions_updated'),     'chat_sessions', ['updated_at'], unique=False)
    ...
    op.create_index(op.f('ix_messages_id'),         'chat_messages', ['id'], unique=False)
    op.create_index(op.f('ix_messages_message_id'), 'chat_messages', ['message_id'], unique=True)
    op.create_index(op.f('ix_messages_session_id'), 'chat_messages', ['session_id'], unique=False)
    op.create_index(op.f('ix_messages_created'),    'chat_messages', ['created_at'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_messages_created'),    table_name='chat_messages')
    op.drop_index(op.f('ix_messages_session_id'), table_name='chat_messages')
    op.drop_index(op.f('ix_messages_message_id'), table_name='chat_messages')
    op.drop_index(op.f('ix_messages_id'),         table_name='chat_messages')
    op.drop_table('chat_messages')

    op.drop_index(op.f('ix_sessions_updated'),    table_name='chat_sessions')
    op.drop_index(op.f('ix_sessions_user_id'),    table_name='chat_sessions')
    op.drop_index(op.f('ix_sessions_session_id'), table_name='chat_sessions')
    op.drop_index(op.f('ix_sessions_id'),         table_name='chat_sessions')
