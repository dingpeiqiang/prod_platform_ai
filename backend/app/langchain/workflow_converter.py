...
class WorkflowConverter:
    ...
    @classmethod
    def _load_ontology_rules(cls, ontology_code: str) -> Dict[str, Any]:
        """从本体加载业务规则（带缓存）"""
        if ontology_code in cls._ontology_rules_cache:
            return cls._ontology_rules_cache[ontology_code]
        
        result = OntologyService.get_business_rules(ontology_code)
        if result["success"]:
    ...
    @classmethod
    def convert(cls, frontend_data: Dict[str, Any], workflow_id: str, workflow_name: str) -> Dict[str, Any]:
        """将前端格式转换为后端格式
        ...
        for node in nodes:
            ...
            step = cls._build_step(node, edges, node_map)
    ...
    def _build_step(cls, node: Dict[str, Any], edges: List[Dict[str, Any]], 
                    node_map: Dict[str, Dict]) -> Dict[str, Any]:
        """构建单个步骤配置
