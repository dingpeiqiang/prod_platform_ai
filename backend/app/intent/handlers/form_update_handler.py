...
class FormUpdateHandler(BaseIntentHandler):
    ...
    async def handle(self, ctx: IntentContext) -> AsyncGenerator[str, None]:
        ...
        yield thinking(f"🔄 识别到表单更新请求: {form_name or detected_form_code}", result={
        ...
        if extracted:
            yield thinking(f"📝 提取到 {len(extracted)} 个待更新字段", result={
                "fields": extracted,
                "count": len(extracted)
            })
        else:
        ...
        yield thinking(f"📊 更新置信度: {confidence:.0%}", result={
