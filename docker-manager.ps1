...
# ==============================================
# 注意：版本号由脚本在构建时自动递增，并同步更新 Sitech.BJ.Dockerfile.* 中的 FROM
$Config = @{
    BackendBase = @{
        ImageName     = "prod-platform-backend-base"
        ImageTag      = "2.7"  # 构建时自动递增版本号
...
# ==============================================
# Version Auto-Increment Helper Functions
# ==============================================

function Get-NextImageTag {
    <#
    .SYNOPSIS
    版本号最后一个数字段递增：2.7 -> 2.8，3.2.1 -> 3.2.2
    #>
    param([string]$CurrentTag)
    $parts = $CurrentTag -split '\.'
    $lastPart = $parts[$parts.Count - 1]
    if ($lastPart -match '^\d+$') {
        $parts[$parts.Count - 1] = ([int]$lastPart + 1).ToString()
        return $parts -join '.'
    }
    return "$CurrentTag.1"
}

function Update-ScriptImageTag {
    <#
    .SYNOPSIS
    将递增后的版本号持久化写回 docker-manager.ps1 的 $Config 定义
    #>
    param(
        [string]$ImageName,
        [string]$NewTag
    ...
    $blockMap = @{
        "prod-platform-backend-base"   = "BackendBase"
        "prod-platform-frontend-base"  = "FrontendBase"
    }
    $block = $blockMap[$ImageName]
    if (-not $block) { return }
    ...
    if ($newContent -ne $content) {
        ...
        [System.IO.File]::WriteAllText($scriptPath, $newContent, $utf8Bom)
        Write-Host "[SYNC] $scriptPath 中 $ImageName 的 ImageTag 已更新为 $NewTag" -ForegroundColor DarkGray
    }
}

function Update-AppDockerfileFrom {
    <#
    .SYNOPSIS
    同步更新应用镜像 Dockerfile 的 FROM 基础镜像版本号
    #>
    param(
        [string]$AppDockerfilePath,
        [hashtable]$BaseConfig,
        [string]$NewTag
    )
    $appPath = Join-Path $ProjectRoot $AppDockerfilePath
    if (-not (Test-Path $appPath)) {
        Write-Host "[WARN] 未找到应用镜像 Dockerfile: $AppDockerfilePath" -ForegroundColor Yellow
        return
    }
    $content = [System.IO.File]::ReadAllText($appPath)
    $newFrom = "FROM $($BaseConfig.Registry)/$($BaseConfig.Namespace)/$($BaseConfig.ImageName):$NewTag"
    $pattern = '(?m)^FROM\s+\S+'
    $newContent = [regex]::Replace($content, $pattern, $newFrom)
    if ($newContent -ne $content) {
        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
        [System.IO.File]::WriteAllText($appPath, $newContent, $utf8NoBom)
        Write-Host "[SYNC] $appPath 的 FROM 已更新为 $newFrom" -ForegroundColor DarkGray
    }
}
...
function Add-VersionBump {
    <#
    .SYNOPSIS
    递增基础镜像版本号并同步持久化（脚本配置 + 应用镜像 FROM），返回新版本号
    #>
    param(
        [hashtable]$BaseConfig,
        [string]$AppDockerfilePath
    )
    $newTag = Get-NextImageTag -CurrentTag $BaseConfig.ImageTag
    $BaseConfig.ImageTag = $newTag
    Update-ScriptImageTag -ImageName $BaseConfig.ImageName -NewTag $newTag
    Update-AppDockerfileFrom -AppDockerfilePath $AppDockerfilePath -BaseConfig $BaseConfig -NewTag $newTag
    Write-Host ""
    Write-Host "[VERSION] 基础镜像版本自动递增至: $newTag" -ForegroundColor Green
    return $newTag
}
    ...
    switch ($choice) {
        "1" {
            ...
            if (Test-DockerAvailability) {
                Add-VersionBump -BaseConfig $Config.BackendBase -AppDockerfilePath "docker/Sitech.BJ.Dockerfile.backend"
        ...
        "4" {
            ...
            if (Test-DockerAvailability) {
                Add-VersionBump -BaseConfig $Config.FrontendBase -AppDockerfilePath "docker/Sitech.BJ.Dockerfile.frontend"
