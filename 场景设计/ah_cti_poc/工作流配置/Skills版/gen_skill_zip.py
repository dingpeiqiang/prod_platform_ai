...
TOOLS = {
    ...
    "save_node_result": ("POST", "/api/v1/appstore/result/save", "节点结果存储：req_id(必填,plan_id或execution_id)/node_name(必填)/result_json(必填)/status → code/msg/record_id；同键覆盖"),
...
| 2 | 确认门禁 | 回复"确认执行/同意/OK"或具体修改意见或"取消"（存在未确认 plan_id 时） | 见「能力2」 |
...
5. 分支：
   ...
   - 无待补充字段：返回 plan_id，结尾固定提示："请核对以上执行方案：回复【确认执行】将自动串行执行 智能配置→稽核→资费校准→自动测试 四个环节；如需调整请直接说明修改意见。"
...
门禁铁律：plan_id 不存在时提示"当前没有待确认的执行方案，请先描述需求生成执行方案"；**未写入确认标记绝不触发配置落地**。
...
### 续跑机制（用户回复【重新执行】）
携 execution_id + 失败阶段编码重入：
1. `query_node_result`（req_id=execution_id，node_name=config/spec/fee/test）回放已成功阶段结果，**已成功阶段的写接口绝不重复调用**；
