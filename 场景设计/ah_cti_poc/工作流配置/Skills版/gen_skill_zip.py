...
def tools_ref():
    lines = ["# 自研插件工具契约（模拟结果输出，种子数据=《产品信息.txt》18个销售品）", "",
             "> 基地址：%s ；全部工具契约与《自研插件集V1.6》一致（V1.7 增加工具层硬校验），替换真实实现时契约不变。" % BASE_URL,
              "> 能力1~4 需存储类工具：req_id 规范（V1.7 统一键）——执行方案与执行主干共用单键 req_id（PLAN+yyyyMMddHHmmss+3位随机数，**由执行方案拆分代码节点以系统时钟生成（datetime.now + 3位随机数），每次分析重新生成、保证唯一、LLM 不参与生成**，禁止沿用示例值/历史值）；node_name 取值：requirement(执行方案)/CONFIRMED(确认标记)/config(智能配置)/spec(稽核)/fee(资费)/test(测试)/report(上线报告)。",
...
# 信息完整性与来源规则
字段三态：原始需求 / 可AI补全 / 待补充。
- 补全策略（本体+相似产品组合，K6《产销品字段本体》为稳定性锚点）：
  逐级降级取值：原始需求 → 相似产品（最高相似度） → 本体默认值 → 待补充，
  禁止跳级虚构。字段形态必须符合 K6 本体约束：
  产品属性=基础/可选/增值；收费方式=按月/按量/一次性（月付/包月归一为按月）；
  渠道类型=实体渠道/电子渠道/直销渠道；适用地区=全国（不含港澳台）/指定省份；
  计费周期=自然月；销售品状态=在售/待上线（新需求一律填待上线）；
...
2. 补全规则（严格，本体+相似产品组合）：**逐级降级取值：原始需求 → 相似产品（最高相似度） → K6本体默认值 → 待补充，禁止跳级虚构**；字段形态必须符合 K6《产销品字段本体》（产品属性=基础/可选/增值；收费方式=按月/按量/一次性；渠道类型=实体渠道/电子渠道/直销渠道；计费周期=自然月；销售品状态新需求一律填待上线；月付/包月归一为按月；数值带单位）；**仅当套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到时→缺失字段填"待补充"，禁止推理、禁止从相似产品照搬**（任一类资源已提取到则其余资源字段不算缺失），列入 pending_fields；**产品编码禁止补全（由智能配置环节落地后生成，value填"由智能配置生成"）**；**其余缺失字段→取最高相似度产品对应值，来源"AI补全"**；来源仅"原始需求/AI补全"两种。
...
- 知识库 K6《产销品字段本体》：四类18字段的类型/枚举/格式/默认规则/兜底口径定义，字段补全稳定性锚点（能力1引用，逐字段核对）。
"""
