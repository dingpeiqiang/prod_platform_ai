...
## 上下文与状态（贯穿所有能力）
- req_id：执行方案存储 key（V1.7 统一键，PLAN+yyyyMMddHHmmss+3位随机数，**每次新生成必须实时取当前时间，禁止沿用示例值或历史值**；执行方案、确认标记与各环节结果同键存储）。由 LLM 按当前时刻生成，全局唯一不重复；修改执行方案场景沿用原值覆盖写。
...
3. 生成执行方案 JSON（req_id/fields/similar_offers/pending_fields；fields 每项含 field/value/source），req_id 格式 `PLAN`+yyyyMMddHHmmss+3位随机数（**取当前真实时刻生成，禁止照抄示例或历史值；仅修改执行方案场景沿用原值**）；调用 `save_node_result`（req_id=该键，node_name=requirement，result_json=方案JSON，status=ok）保存。
