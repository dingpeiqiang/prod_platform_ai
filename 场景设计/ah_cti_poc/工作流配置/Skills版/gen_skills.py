...
s2.append(plugin_node(104, "写入确认标记", "save_node_result",
    ...
    "save_node_result",
    [inp("req_id", "执行批次标识=execution_id（EXE+yyyyMMddHHmmss+2位序号，与plan_id同轮生成）", ref_block=nid(101), ref_rel="plan_id"),
...
s3.append(start_node(201, [
    ...
    inp("execution_id", "执行主干批次号 EXE+yyyyMMddHHmmss+2位序号（续跑时传入，首跑自动生成）", required=False),
