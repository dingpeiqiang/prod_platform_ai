...
s1.append(llm_node(4, "字段映射与补全",
    "基于业务要素与相似产品列表，映射为实际配置字段并补全：\n要素：{elements_json}\n相似产品：{similarOfferList}\n补全策略（本体+相似产品组合，K6字段本体为稳定性锚点）：\n逐级降级取值：原始需求 → 相似产品（最高相似度） → 本体默认值 → 待补充，禁止跳级虚构；\n字段本体约束（必须逐字段核对，违反即重填）：产品属性枚举 基础/可选/增值（套餐类默认基础、权益包默认增值）；收费方式枚举 按月/按量/一次性（月付/包月归一为按月）；渠道类型枚举 实体渠道/电子渠道/直销渠道（可多选，无参照默认三者全选）；适用地区枚举 全国（不含港澳台）/指定省份（默认全国）；计费周期枚举 自然月（默认）；销售品状态枚举 在售/待上线（新需求一律填待上线，禁止取相似产品的在售）；优惠条件/优惠期无优惠填无；副卡规则无参照填不允许办理副卡；退订规则默认口径：允许退订，次月生效，当月费用不退还（相似产品同值优先）；产品名称必须符合K1命名模板；数值类字段必须带单位（GB/分钟/条/元）；\n待补充触发（仅两种）：仅当套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到时，才将缺失字段填\"待补充\"（禁止推理，禁止从相似产品照搬；任一类资源已提取到则其余资源字段不算缺失）；产品编码禁止补全（由智能配置环节落地后生成，未提取到时value填\"由智能配置生成\"）；其余缺失字段取最高相似度产品对应值（须符合本体枚举/格式），来源\"AI补全\"；来源只允许\"原始需求\"/\"AI补全\"两种。\n输出要求（单一出参 plan_output）：\n按以下格式输出，第一行原样输出标签 plan_output:，随后紧跟一个JSON对象（以{开头、}结尾），除该标签行外不得输出任何其他文字、代码块或说明：\nplan_output: {\"plan_json\":..., \"plan_md\":..., \"pending_fields\":...}\n该JSON对象固定包含以下3个键：\n1. plan_json：执行方案JSON对象，含 req_id/fields/similar_offers/pending_fields 四个键，fields内每项含 field/value/source；req_id 键留空字符串（由后续代码节点统一生成，禁止自行生成）；\n2. plan_md：执行方案Markdown表格字符串（以|字段分类|开头，固定4列：字段分类/字段名称/字段值/来源），表格内换行使用\\n转义，确保整个输出是合法JSON；\n3. pending_fields：待补充字段名称数组，无待补充时为空数组[]。",
