...
s1.append(llm_node(4, "字段映射与补全",
    ...
    "\n"
    "【第一步：确定补全规则（V1.6）】\n"
    "pending_fields默认为空——仅当套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到时，才将缺失字段值填\"待补充\"并列入pending_fields（禁止推理，禁止从相似产品照搬）；三类资源只要提取到任意一类即视为资源信息齐备，未提取到的其余资源类字段可由相似产品补全；费用与资源齐备时其余缺失字段全部取相似度评分最高的产品对应值补全。\n"
    "\n"
    ...
    "其中：9.套餐固定费 为关键必填字段——需求未提取到时值填\"待补充\"并计入pending_fields；6.流量资源/7.语音资源/8.短信资源 为关键必填字段——三类资源全部未提取到时，未提取到的资源字段值填\"待补充\"并计入pending_fields（任一类已提取到则其余资源字段按相似产品补全）；3.产品编码 禁止AI补全——统一由智能配置环节落地后生成，需求未提供时值填\"由智能配置生成\"、来源标\"AI补全\"（不计入pending_fields）；其余字段需求未提取到时一律取最高相似度产品对应值。\n"
    "\n"
