...
s1.append(llm_node(4, "字段映射与补全",
    ...
    "\n"
    "【第二步：组合补全策略（本体+相似产品，V2.0 稳定性锚点）】\n"
    "补全值必须先满足 K6《产销品字段本体》的字段形态约束，再从相似产品取值，逐级降级：原始需求 → 相似产品（最高相似度） → 本体默认值 → 待补充，禁止跳级虚构。\n"
    "字段本体约束（必须逐字段核对，违反即重填）：\n"
    ...
    "取值顺序：①需求原文有值→直接采用；②需求无值→取最高相似度产品对应字段值（须符合上述枚举/格式）；③相似产品亦缺失→按本体默认值；④仅套餐固定费与三类资源按第一步判待补充。每级降级来源统一标\"AI补全\"。\n"
    "\n"
    ...
    "其中：9.套餐固定费 为关键必填字段——需求未提取到时值填\"待补充\"并计入pending_fields；6.流量资源/7.语音资源/8.短信资源 为关键必填字段——三类资源全部未提取到时，未提取到的资源字段值填\"待补充\"并计入pending_fields（任一类已提取到则其余资源字段按相似产品补全）；3.产品编码 禁止AI补全——统一由智能配置环节落地后生成，需求未提供时值填\"由智能配置生成\"、来源标\"AI补全\"（不计入pending_fields）；其余字段按第二步组合补全策略取值。\n"
    "\n"
...
files["wf_sub_01_需求分析.json"] = workflow(
    "产销品-需求分析", "子工作流1：需求分析（执行方案生成）。需求理解→相似产品查询（自研模拟，18销售品种子）→字段映射与AI补全（pending_fields默认为空，仅套餐固定费未提取到或三类资源全部未提取到时待补充；产品编码不补全由智能配置生成；其余AI补全）→待补充项判断（无待补充→保存执行方案→确认结束；有待补充→补充提示结束）。", "wf_sub_01", s1, e1)
