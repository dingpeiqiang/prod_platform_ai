...
- **status=通过 时按审批类型区分衔接（V9.1）**：
  - **需求工单审批**（出参 approval_type=requirement）：输出"需求工单审批已通过 ✅"，提示"**建议处理：可输入'开始配置'进入【需求分析】。**"（衔接 flow-A，配置流程第一个交互点）；
  - **上线审批**（出参 approval_type=launch）：输出"审批已通过，销售品上架完成 ✅"，提示"**建议处理：可输入'确认上线'进入【监控运维】。**"（衔接支线D-3）；
