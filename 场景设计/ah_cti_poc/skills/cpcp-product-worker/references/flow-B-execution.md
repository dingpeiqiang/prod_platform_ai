...
> 对应原子工作流 wf_sub_02→03→05→04。用户确认配置后**一次跑完五个环节（智能配置→配置规格稽核→资费校准→自动测试→受理验证），中途不停顿**；仅环节失败时中断。各环节共用 req_id；每环节成功后存储（node_name=config/spec/fee/test）并立即打印结果（环节5 受理验证复用环节4 测试结果，不新增存储键）。
...
- 配置上线脚本下载链接：{{script_url（出参缺失时省略本行）}}

**建议处理：可输入"执行稽核"进入【配置规格稽核】。**
```
- script_url 引用纪律：链接逐字引用出参 `script_url` 原文（相对路径时拼接 BASE_URL 前缀），禁止自行构造或改写参数；脚本内容为后端生成的 CRM/billing 落库 SQL（模拟），仅在用户要求查看时按下载内容原文回显，禁止转述加工；
...
- 存储 node=fee；
- **输出模板（比对表固定 8 项，"套餐描述"列逐字引用执行方案字段值，"计费配置描述"列逐字引用出参比对明细；出参无对应比对项时该行整体省略，禁止编造系统侧值）**：
```
已完成资费校准及计费规则验证。
...
| :--- | :--- | :--- | :--- |
| 套餐月租 | {{套餐档位，如 29元}} | {{计费配置值（含折算规则括注，如 29元（首月按天折算））}} | {{一致/不一致}} |
...
- 受理验证小节数据源=出参 orderId/offerInstId 与逐受理场景（S_O_TC/S_ADD_CARD/S_U_TC）结论及关键测点比对，逐字引用禁止加工；受理凭证为空时按 E14 固定文案标注，不中断。
