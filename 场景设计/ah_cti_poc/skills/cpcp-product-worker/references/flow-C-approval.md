...
> 对应原子工作流 wf_sub_06。执行主干四环节全部成功且用户明确确认后，串行自查五类结果、生成上线校验看板并推送审批流。
...
**风险检查：** {{逐字引用：配置完整性/资费风险/计费风险/受理风险结论}}

**AI审批建议：{{全部通过→"建议上线"；任一环节未通过→"暂缓上线"}}。**
```
- 看板各检查项与五类自查结果一一对应：需求完整性=requirement 存在且无待补充；配置规格稽核=spec pass；资费校准=fee pass；自动测试=test 通过；受理验证=orderId/offerInstId 非空且受理场景通过；
