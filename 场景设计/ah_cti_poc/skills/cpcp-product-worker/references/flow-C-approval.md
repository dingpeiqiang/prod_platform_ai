...
> - **V13.0 审批通过即自动上线（无需二次确认）**：上线审批（approval-type=launch）**通过即视为销售品已自动上线**，**不再需要用户二次回复"确认上线"**；流程自动进入**步骤7** 轮询审批状态，`status=通过` 即自动衔接**环节9 监控运维**——输出《监控运维方案》+ 单品运营可视化看板；若审批尚未通过，则以上述环节8 标题头 + 收尾块结束（引导回复【查询审批进度】核验），**不得要求用户回复【确认上线】触发**（该意图与支线D-3 已移除）。

### 步骤7：审批通过自动上线 → 自动衔接环节9 监控运维
> **V13.0 新增**：审批通过即自动上线，**无需用户二次确认**。轮询审批状态（模拟服务审批提交 10s 后自动流转为"通过"，不会一直停留在"审批中"）：
```bash
python scripts/cpcp_api.py approval_status --approval-id "<approval_id>" --product-id "<product_id>"
```
- 出参 `status=通过` 且 `approval_type=launch` → 销售品**已自动上线**，自动输出《监控运维方案》+ 单品运营可视化看板（环节9，模板与口径见 flow-D 支线D-1「V13.0 审批通过自动上线·监控运维方案」步骤），**流程顺利走完第9环节，无需用户回复【确认上线】**；
- 出参仍为待审/驳回 → 不擅自上线，输出环节8 收尾块引导（`> **建议处理：** 上线审批已提交并轮询中，审批通过后将自动上线（可回复【查询审批进度】核验当前进度）`）后结束，等待用户后续【查询审批进度】核验。

## 禁止事项
- 严禁在四环节结果不齐时调用审批推送（后端会拒绝，且属调度违规）；
- 严禁未经用户明确确认自动发起审批（发起审批仍需用户确认【发起审批】/【提交审批】）；
- 看板检查项结果必须与自查出参一一对应，禁止全 ✅ 敷衍（任一环节异常时应早已中断，不应走到本程序）；
- **禁止省略环节8 收尾固定块**：审批推送成功后每段输出必须以"**建议处理：**"收尾块结束（SKILL.md 纪律5.1），禁止以审批矩阵表格收尾；异常场景（❌ 执行失败）收尾块改按 E16/E29 引导（回复【重新执行】）；
- **禁止再要求用户二次确认上线**：审批通过后不得再让用户回复【确认上线】（该意图与支线D-3 已移除），直接自动衔接环节9 监控运维。
