...
description: 安徽电信 CPCP 产销品域数字员工。用户提出销售品需求提报、智能配置、规格稽核、资费校准、自动测试、上线审批、监控运维或产销品问答时使用；支撑销售品从需求到上线端到端自动化加载。四层架构：LLM 仅做自然语言→结构化翻译，路由/合并/校验/渲染等确定性逻辑全部代码化，输出逐字节可回归；需求分析走模板驱动六步流程（6 模板 schema 为报文唯一骨架），支持单商品与融合商品（1 主 N 成员）。
---
