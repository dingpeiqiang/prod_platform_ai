...
- **需求工单审批通过后**，用户触发**配置流程**，按大交互点依次执行：**需求分析 → 销售品智能配置（配置规格稽核/资费校准/销售品自动测试/受理验证）→ 上线审批 → 监控运维**；**上线审批（环节8）通过即视为已自动上线，直接自动衔接监控运维（环节9），不再需要用户二次回复"确认上线"**；
...
**编排铁律（V5.0 延续；V10.0 扩展到 flow-A）**：执行主干四环节的串行顺序、成败判定、存储落盘、续跑回放已收敛为确定性脚本 `scripts/run_pipeline.py`（状态机）；**需求分析（环节2,flow-A）的确定性步骤（②相似检索双源降级/③取模板/⑤合并/⑤.5校验/⑦保存/⑥渲染）已由 `scripts/run_requirement.py` 三阶段状态机（products→prepare→merge-save）收敛**，模型只做①④两处 LLM 翻译并读最终渲染结果。**禁止模型逐步调用单环节脚本自行编排**。**V2.0 收敛增强**：run_requirement 纯本地步骤（merge_nested/validate_elements/render_table 直import，identify_products/derive_flat24 内联）不再子进程派生；stdout 紧凑化（渲染表/推理/全量结果落盘 `requirement_result_<req_id>.json` 等工件，模型读工件而非内联长文）；阶段状态持久化（`requirement_state_<req_id>.json`）+前置校验防错舞台调用；E31 增量补提 `--elements-supplement` 与 elements 叠层合并免整段重提。详见 flow-A-requirement.md「编排收敛」节。
...
> | 8 | **上线审批** | flow-C（程序C） | 四环节全成功且用户确认发起 | 审批单号/审批矩阵（approval-type=launch，区别于环节1 需求工单审批）；**审批通过即视为已上线，自动衔接环节9 监控运维，无需二次确认上线** |
> | 9 | **监控运维** | flow-D 支线D-2（运行监控/闭环） | 审批通过后自动衔接 或 用户查询监控 | 摘要+可视化URL+根因闭环；审批通过即输出监控运维方案 |
>
...
| 查询运维程序 | `references/flow-D-query-ops.md` | intent 为 QUERY_APPROVAL/QUERY_MONITOR/QUERY_OFFER 时 |
...
| 存量产品目录 | `references/存量产品目录_清洗后.json`（**随 skill 打包内置在 references/ 下，flow-A 步骤② 一律经 `query_offer` 脚本检索，禁止 LLM 自行定位/手读该 JSON**） | flow-A 步骤② 本地相似检索主源 |
| 知识库 K1~K5 | `references/K1规范|K2资费|K3测试|K4存量|K5存量报文/` | 按 dispatcher `kb_target` 或对应流程读取指令（K5存量报文=步骤② 相似品报文源，取 `references/K5存量报文/<offer_id>.json`） |
| 工具契约 | `references/tools-contract.md` | 仅脚本参数/出参不确定时 |
| 本体推理接入方案 | `场景设计/ah_cti_poc/方案/本体推理接入CPCP模板轨-设计.md`（**不在 skill 包内**，仅作设计依据参照） | flow-A 步骤⑤.5 嵌套本体校验闸接入时的设计依据（对接 Java 推理平台，不重造本体） |
...
5. 敏感资费与配置明细仅展示摘要；**输出结构铁律**：凡流程输出一律按环节分块整理，**每环节以 [九环节总表] 的【环节{全局序号}/9·{环节全名}】统一标题头开头**（序号/全名见上方九环节总表，禁止缩写、禁止用旧"环节1/4·智能配置"的短名），标题头下接执行结果、关键数据、下一步建议三要素，环节间用 `---` 分隔；标题头形态封闭（仅"✅ 执行成功"/"❌ 执行失败"两种）；执行主干四环节（3~6）全部输出完毕必须输出文末【执行主干全部完成】汇总块（任一环节失败改输出【异常】模板，禁止输出任何汇总表格）；
5.1 **收尾固定块（V11.0，全环节强制，防"回复结尾没有下一步动作"）**：**凡给用户的流程输出（环节1~9 任一环节、查询类回显、审批结果、监控摘要），正文末尾必须输出一个"下一步处理动作建议"固定收尾块**，独立成块、加粗突出，形态封闭为：
   ```
   > **建议处理：** {{一句话动作建议}}（可回复【{{明确可回复的触发词}}】{{可选补充动作}}）
   ```
   规则：① 该块必须是整条回复的**最后一个内容块**（可视化面板/下载图标行之后仍须补该块），**禁止以表格、统计值、"待补充字段：无"、分隔线或空行收尾**；② 触发词**只能取两类**：(a) `dispatcher.py` 规则表已可直接命中的意图词（如【查询监控】/【查询审批进度】/【确认配置】/【重新执行】/【修改执行方案】/【发起需求审批】/【提交审批】等，**禁用【查询工单】【关闭工单】【存量合规扫描】等 dispatcher 未定义的自造词**）；(b) 各 flow 已明确定义的环节衔接词（如【跳过审批】/【开始配置】/【需求分析】/【继续】——由对应 flow 的意图分类/门禁条款承接，不得脱离该 flow 自行发明新词）；**禁止给出既非 dispatcher 词表、又非任一 flow 定义的自造触发词**；③ 等待用户表态的场景（决策点/门禁/待补充）须在块内附加等待态说明（如"回复后我才会继续执行"），**不得擅自代替用户推进下一环节**；④ 异常/失败场景的收尾块按异常矩阵引导（回复【重新执行】/【修改执行方案】），不得给出下一环节引导；⑤ 每环节收尾块措辞以各 flow 输出模板给出的为准，模型禁止改写触发词或省略该块。
...
13. **运营可视化看板出口纪律（V8.0；V9.2；V9.3 改走 xsbot-panel 外链；V13.0 输出强制 JSON）**：运行监控（支线D-2 文本摘要）与审批通过后的监控运维方案（环节8 通过即自动上线衔接）输出后**均须**输出单品运营可视化看板——**输出结果必须是 JSON，且必须以 `xsbot-panel` 代码围栏包裹（` ```xsbot-panel ` 围栏）**。**xsbot-panel 输出 JSON 强制按以下结构整段逐字输出，禁止改为纯文本/纯 URL/纯表格拼凑、禁止手写页面 HTML/图表载荷、禁止内联监控数据**：
   ```xsbot-panel
   ...
   ```
   字段约束：`message_id` 取会话 `chat_id`（会话未出现过时不允许臆造为 0/占位）；`url` 的 `busiId` 固定为 `SHI_MING_JI_HUO`、`chatId` 取 `chat_id`，URL 基址 `http://10.88.158.111:10002/gzdg/orderForm/` 保持不变；`title` 固定为「实名激活」；`panel` 固定 `right`、`mode` 固定 `external`。外部页面渲染完全由前端完成，LLM 只负责将 `{chat_id}` 替换为会话消息 ID。
14. **交互提示 Markdown 纪律（V9.5）**：**全流程一律使用 Markdown 文本输出，禁止调用 `render_a2ui`（及任何表单/组件渲染工具）**。信息展示、文档/报告、结果呈现、数据回显均直接输出脚本出参文本（如需求提报单输出 `report_text`）；**需要用户选择下一步操作时，用 Markdown 列出可选项与对应回复指令**（如"请回复【查询】或【配置】"、"请回复【发起需求审批】或【跳过审批】"），禁止以组件/表单代替文本提示；不得用即兴 HTML 或图表载荷拼凑，选项文案必须明确可回复。
...
- **执行主干编排 `python scripts/run_pipeline.py --req-id <req_id> --workdir <会话可写目录> --confirmed [--stage prepare|poll] [--resume --fail-node <STAGEx>]`**（V5.0：四环节确定性状态机；出参 state JSON 中 nodes[].result_file 指向各环节出参工件；**V12.1：cpcp_api 调用与测试轮询改为进程内直调 `_http`/cmd_*，消除次生子进程启动开销**；**V12.3 分段执行：新流程一律走 `--stage prepare`（跑完 config→spec→fee→发起测试即秒级返回 `next_action=POLL`+`globalId`）→ 循环 `--stage poll`（单次查进度，秒级；未完成返回 POLL，完成返回 APPROVAL_GATE）——每条命令均秒级返回，根治长命令被 Agent 工具转后台导致的"等通知"误判；模型循环 poll 直到 next_action != POLL**；`--stage all` 仅作旧调用兼容）；
- **需求分析编排 `python -X utf8 "scripts\run_requirement.py" --stage <products|prepare|merge-save> --req-id <req_id> --workdir <会话可写目录> ...`**（V10.0：flow-A 确定性步骤收敛状态机；三阶段，模型仅在阶段间做①④ LLM 翻译；**V2.0**：纯本地确定性步骤直import 不再子进程、stdout 紧凑化——渲染表/推理/全量结果落盘 `plan_md_<req_id>.md`/`explain_<req_id>.json`/`requirement_result_<req_id>.json`（模型读 `requirement_result_<req_id>.json` 的 `tables/pending_required` 而非内联长文）、`requirement_state_<req_id>.json` 阶段持久化+前置校验、E31 增量补提 `--elements-supplement` 叠层合并；退出码 0=出口B已保存/1=出口A待补充/2=前置缺失/3=E30/4=E31/5=E32/6=E33/7=E1；详见 flow-A-requirement.md「编排收敛」节）；
...
- **存量产品信息查询（flow-D 支线D-4，本地只读）**：`python scripts/cpcp_api.py query_offer --product-id <9位编码>` 或 `--name <产品名称>` / `--keyword <描述关键词>`（读取 skill 包内置 `references/存量产品目录_清洗后.json` + `references/K4存量/`，纯只读不改写）→ 出参 `matched[]`（含 offer_id/name/product_type/tier/template/members + k4_text 档案原文；`status=dup` 自动透传 `duplicate_of` 指向 active 品）；命中→按出参结构化回显存量产品信息，未命中（matched=[]）→ 追问核对名称/ID，禁止编造字段、禁止转需求提报/配置流水线；
...
- **输出前程序化校验（纪律1 强制兜底；V12.0 单次全量）**：执行主干四环节跑完后，模型**一次性渲染完整输出文档**（环节3/4/5/6 + 环节7受理验证 + 汇总块），仅**一次**运行 `python scripts/validate_output.py --node all --workdir <会话可写目录> --req-id <req_id> --output-file <输出草稿路径>` 对全文做单次全量校验（内部自动按环节命名读取工件：config→`config_result_<req_id>.json`，spec/fee/test→`result_<node>_<req_id>.json`）；校验 FAIL → 按问题清单修正后重新校验，通过后再一次性输出（校验报错本身不中断主干，按 E24 输出失败说明）。**禁止逐环节独立渲染/独立校验/独立输出**（那会制造多次 LLM 渲染与多次校验子进程调用，属多余操作）。**"一次性渲染"仅指渲染/校验/输出各一次，各环节必须仍按其模板完整结构化分块呈现（【环节N/9·全名】标题头 + 明细表 + 下一步引导行，`---` 分隔）**，禁止因合并渲染而丢失任一环节的结构化分块/标题头/表格。单环节细分定位仍支持 `--node <config|spec|fee|test> --result-file` 兼容用法。
