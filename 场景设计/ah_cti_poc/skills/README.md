...
└── cpcp-product-worker/                       # 产销品数字员工技能包（唯一技能）
    ...
    └── references/
        ...
        ├── K2资费/   （2 份）                  # 资费规则（原 K2）
        ├── K3测试/   （2 份）                  # 测试规范（原 K3，V2.0 正式版：三大验证 31 条固定用例 + 9 章节报告模板）
...
| wf_sub_04 节点6（K3） | 程序B 环节4：生成报告前读取 K3测试_销售品自动化测试报告模板_V2.0.md（正式版 9 章节）+ K3测试_销售品自动化测试用例设计规范_V2.0.md（三大验证 31 条固定用例与 P0/P1/P2 判定） |
...
- [ ] 程序B 环节4 测试报告对齐正式版模板：9 章节结构 + 三大验证（ACC 12/BILL 10/CUST 9）+ P0/P1/P2 缺陷分级 + 整体上线结论三选一（任一 P0 ❌ → 禁止上线）；
- [ ] 程序B 环节4 测试报告含受理验证结论（orderId/offerInstId 为空时标注人工核实）；
- [ ] 程序B 环节4 报告下载链接：test_result 出参 `report_url` 逐字输出 + `download_test_report` 下载报告文件成功（后端 GET `/test/offer/report` 未归档时 404、脚本非 0 省略下载行不中断）；
- [ ] 18 销售品全量兼容回归（原附录 E）；
...
- **业务口径零改动**：3 模块/9 分类 24 字段、待补充判定（仅价格类套餐档位）、来源两态（原始需求/AI补全）、异常矩阵 E1~E23 全部原样迁移。
