...
### 步骤3：同构键值合并
```bash
python scripts/cpcp_api.py merge_fields --fields-json "<elements_json>" --offer-json "<similar_offer 出参>"
```
脚本按 field 名逐字段确定性对齐，模型不再手工合并（四层架构：确定性逻辑一律代码化）：
...
- 出参保留 category（供 build_plan 模块归并），脚本 `offer_json` 兼容扁平 fields / {fields:[...]} / 组结构 / 原始 offer_group(preset) 多种形态；
- 可辅助核对：读取 `references/K4存量/K4存量_产品信息{similarOfferId}_V1.0.md` 单文件（禁止作为字段来源覆盖用户原始需求）；
- **V4.0 融合组：合并逐成员执行**——主套餐与各成员各自对位 offerInfo/offer_group（成员 preset 值取自 offer_group.members[].preset，source=AI补全）；**价格字段禁止跨成员照搬**（主套餐档位 ≠ 宽带月功能费 ≠ 副卡月功能费，各自独立判定，脚本按成员独立处理）。
...
> **注（V6.1）**：`ontology_reason` 出参 `fields_json` 剥离开 `category`（仅 field/value/source）。`build_plan` 已内置「字段→分类」注册表（`FIELD_CATEGORY`）按 24 字段名确定性回填 category，plan_md 模块/分类列不会因此变空串——**无需手工回填，禁止在步骤4后手工补 category 再喂 build_plan**（脚本自愈）。
