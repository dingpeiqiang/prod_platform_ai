...
> V3.2 履历 | 2026-09-19 | **执行主干由"确认后自动串行"改为"逐环节推进"（提示词/文档同步优化，工作流 JSON 与后端零改动）**：① 智能体提示词执行主干纪律重写——用户确认配置后**仅调度首个环节 `wf_sub_02`**，**每环节执行后（无论成功或异常）立即暂停**，输出该环节结果 + 下一步建议，等待用户反馈后再决定下一步；② 环节间不再自动衔接——成功时用户回复【继续】才调度下一环节（02→03→05→04，串行顺序不变），异常时引导【重新执行】（从失败环节续跑）/【修改执行方案】（回到需求分析）；③ 意图映射表新增"继续下一步"行、限制条款改写为逐环节暂停纪律、成功/异常打印模板补充"暂停待反馈"说明；④ 1.1 建设目标、1.2 智能体行、1.3 设计原则、2 章能力映射、6.1 调度模式（含时序图暂停节点）、6.3⑧⑨、6.5.3、7/8/9/10 章与附录 Checklist 同步；⑤ 业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约一律不变 |
...
| 版本 | 日期 | 变更说明 |
| --- | --- | --- |
| V3.2（逐环节推进） | 2026-09-19 | **执行主干由"确认后自动串行"改为"逐环节推进"**：智能体提示词执行主干纪律重写——用户确认配置后仅调度首个环节 `wf_sub_02`，**每环节执行后（无论成功或异常）立即暂停**，输出该环节结果 + 下一步建议，等待用户反馈后再决定下一步；成功时回复【继续】才调度下一环节（02→03→05→04），异常时引导【重新执行】/【修改执行方案】；意图映射表新增"继续下一步"、限制条款改写为逐环节暂停纪律、打印模板补暂停说明；1.1/1.2/1.3/2 章/6.1（时序图）/6.3/6.5.3/7/8/9/10 章与附录 Checklist 同步；工作流 JSON 与后端零改动，业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约不变（详见顶部 V3.2 履历） |
...
**需求提报 → 需求分析（五列模块表格《加载方案》）→ 用户确认 → 【智能配置（配置落地）→ 配置规格稽核 → 资费校准 → 销售品自动测试（含受理验证子集）】逐环节推进（每环节暂停等用户反馈）→ 上线审批（上线校验看板，用户二次确认后发起）→ 监控运维（确认上线后生成监控运维方案）** 的端到端闭环。
...
> V3.2 关键调整：**执行主干由"自动串行"改为"逐环节推进"**——用户确认配置后仅调度首个环节 `wf_sub_02`，**每环节执行后（无论成功或异常）立即暂停**，输出该环节结果 + 下一步建议，等待用户反馈：成功时用户回复【继续】再调度下一环节（02→03→05→04），异常时引导【重新执行】/【修改执行方案】。串行顺序不变，但环节间不再自动衔接，改由用户逐步确认推进。

### 1.2 总体架构（平台能力映射）

| 平台能力 | 在本场景中的角色 |
| --- | --- |
| 智能体（助手） | 产销品数字员工统一入口（对话式 + 流程式调度中枢）；由智能体（LLM）按 3.2 提示词【意图→工作流映射表】**语义识别直调** 11 个子工作流（逐环节推进、每环节暂停等反馈、结果打印、异常引导由提示词【限制】约束），后端硬校验兜底；流程结束后承接**消息查询**（审批进度/监控结果）与**异常处置引导**（重新执行/修改执行方案） |
...
### 1.3 设计原则
1. **子工作流化 + 智能体语义调度**：每个环节封装为子工作流（wf_sub_00~10，req_id 单必填入参自查链路）；无独立意图调度主流程，由智能体（LLM）按 3.2 提示词【意图→工作流映射表】语义识别后**直调**各子流，确定性纪律（确认门禁、逐环节推进不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败）固化在提示词【限制】并由后端硬校验兜底。
...
8. **逐环节推进，每环节暂停等反馈**：用户确认后的执行主干（智能配置→实时稽核→资费校准→自动测试）按固定顺序推进，但**每完成一个环节即暂停**，打印该环节处理结果与下一步建议，**等待用户反馈后再推进下一环节**（成功与异常均暂停）。成功时提示回复【继续】进入下一环节；异常时打印异常节点+异常原因，并引导用户【重新执行】或【修改执行方案】；全部成功后打印各环节成功详情并提示"是否发起上线审批"。
9. **审批二次确认**：上线审批不自动触发，须在执行主干全部成功后由用户确认发起。
10. **人机协同**：用户确认执行方案、确认发起上线审批两个生产/发布决策点保留人工确认；**执行主干每环节完成后暂停等用户反馈**再推进下一环节；流程结束后支持消息查询审批进度与监控运维结果。
...
| --- | --- |
| 智能体常驻提示词（3.2 角色+技能+意图映射表+限制） | 智能体（LLM）按 3.2 节提示词【意图→工作流映射表】（覆盖 需求提报/确认配置/失败续跑/上线审批/审批进度/监控运维/存量查询/存量合规/QNA 等）做**语义识别并直调各子流**；无独立意图调度主流程 |
...
| 平台门禁（确认标记/四环节校验） | 后端硬校验保留不变（5002/四环节门禁/幂等），确认门禁由智能体提示词【限制】保证（确认语义未命中不触发执行主干），防跳步由后端四环节硬校验兜底 |
...
| **用户确认** | 人工确认执行方案，确认后才进入配置落地 | 智能体提示词【限制】识别确认语义（未命中确认不触发智能配置） |
| **执行主干逐环节推进** | 智能配置→稽核→资费→测试按序推进，**每环节完成后暂停**等用户反馈；每环节打印结果+下一步建议，异常打印节点与原因并引导重新执行/修改执行方案 | 智能体语义识别"确认配置"后调度首个环节 `wf_sub_02`，此后按用户反馈逐环节推进 `wf_sub_03`→`wf_sub_05`→`wf_sub_04`；异常按 fail_node 续跑（门禁由后端硬校验） |
...
| **审批进度查询** | 用户发送消息查询审批单当前状态 | 子工作流 `wf_sub_08`（审批进度查询，**双轨**查询）+ `query_approval_status`（按 approval_id/offer_id） |
| **存量产品查询** | 用户发送消息查询存量/在售产品销售品信息（只读） | 子工作流 `wf_sub_09`（存量产品查询，**只读**）+ CODE_OP_QUERY_OFFER + `query_similar_offer`/`query_node_result` 只读查询 |
| **存量合规扫描** | 存量销售品合规扫描（上架合规） | 子工作流 `wf_sub_10`（存量合规扫描）：CODE_OP_SHELF_COMPLIANCE → 后端 `/shelf-compliance` 端点 |
...
| 助手名称 | 产销品数字员工 |
| 功能介绍 | 面向产销品域的数字员工，覆盖需求提报→需求分析（五列模块表格《加载方案》）→用户确认→智能配置→规格稽核→资费校准→自动测试（含受理验证）→上线审批→监控运维的全流程自动化。 |
| 发布范围 | 所有人可见 |
| 图标 | 默认/上传产销品图标 |

### 3.2 提示词（角色 + 技能 + 限制 模式）

> 本节提示词为**智能体意图识别 + 子工作流直调**的权威定义：智能体（LLM）按下方【意图→工作流映射表】语义识别后**直调 11 个子工作流 `wf_sub_00`~`wf_sub_10`**（子流均自带开始节点/req_id 入参/自查链路/结束节点，为完整独立工作流）。执行主干为**逐环节推进模式**：确认配置后先调度 `wf_sub_02`，每环节执行后暂停并输出结果+下一步建议，待用户反馈（【继续】/【重新执行】/【修改执行方案】）后再推进下一环节。确定性纪律（确认门禁、逐环节不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败、审批四环节后端硬校验兜底）固化为下方【限制】。提示词按【角色 + 技能 + 限制】三段式组织。
...
1. 需求提报与分析：理解需求→拆解要素（3 模块/9 分类/24 字段）→补全→生成《加载方案》
   （五列模块表格 + 执行方案 JSON，经节点结果存储保存，返回 req_id）。需求原文最高
   优先、不得虚构；来源仅"原始需求/AI补全"两态；仅套餐档位（价格）缺失且无参照才
   "待补充"（禁推理价格）；套餐编码填"系统待生成"；有待补充则列出并提示补充。
2. 执行主干（确认后逐环节推进，每环节暂停等用户反馈）：识别确认语义（确认配置/确认
   执行/同意/可以/执行吧）→取最近 req_id→先调度 `wf_sub_02` 智能配置；**每完成一个
   环节即暂停**，按"每环节结果输出模板"打印结果（引用出参原文）+ 下一步建议（"建议
   处理：可输入××进入【××】"），**等待用户反馈后才调度下一环节**（成功与异常均暂停）。
   环节顺序 `wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`（受理验证为测试子集随
   结果输出）；环节结果由子流 save_node_result 落库（node_name=config/spec/fee/test）。
   环节成功（status==SUCCESS / pass==1 / test_passed==通过）→提示用户回复【继续】
   进入下一环节；环节异常（status≠SUCCESS / pass≠1 / test_passed≠通过）→打印异常
   环节+原因+关键明细，引导【重新执行】（按 fail_node 续调，已成功环节不重跑）或
   【修改执行方案】。
...
| 确认配置 | 仅调度首个环节 `wf_sub_02`（后续逐环节等【继续】） | req_id=最近值 |
| 继续下一步 | 按当前已完成环节推进下一环节（02→03→05→04） | req_id 沿用；自查回放 |
...
4. 执行主干必须按 `wf_sub_02`→`03`→`05`→`04` 顺序逐环节推进，**每环节执行后必须暂停
   等待用户反馈，严禁连续调用多环节、严禁并行、跳步，严禁凭语义推断成败**（仅依据
   出参 status/pass/valid/test_passed/backend_pending 判定）。
5. 每环节暂停时须输出该环节结果 + 下一步建议；用户未反馈前不得自行推进下一环节；
   已成功环节续跑不重复调用（凭存储回放）。
6. 审批必须四环节全成且用户确认后才发起；审批通过前严禁生成监控运维方案。
7. 异常（含环节失败）时同样暂停，明确给出环节/原因/建议并引导重执行或改方案，
   不得自行重试。
8. 输出结构化：环节名/结果/关键数据/下一步建议；✅ 与统计值须与出参一一对应，
   禁止凑数、虚构，不泄露资费配置明细。
```
...
| 配置项 | 取值 |
| --- | --- |
| 插件 | 挂载第 2.1 节全部插件工具与后端适配端点（既有 13 个工具 + V2.0 新增 7 个适配端点 + 节点结果存储查询插件，共 21 个；确定性逻辑另内嵌为工作流 type=6 代码节点） |
| 工作流 | 挂载 **11 个子工作流 JSON**（`wf_sub_00`~`wf_sub_10`，见 `工作流配置/智能体工作流集V1.6/`，由 `gen_workflows_v2.py` 生成；无意图调度主流程，由智能体按 3.2 节【意图→工作流映射表】语义识别后**直调**） |
...
### 6.1 智能体调度模式（智能体语义识别直调，无意图调度主流程）

> **核心口径**：无独立意图调度主流程，调度职责回归智能体（LLM）——按 3.2 节提示词【意图→工作流映射表】语义识别后**直调** 11 个子工作流 `wf_sub_00`~`wf_sub_10`。常驻提示词精简（角色/技能/映射表/限制），逐环节推进、结果打印、异常引导由【限制】约束；确定性纪律（确认门禁、逐环节不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败）固化为提示词硬性原则，并由后端硬校验（四环节门禁/req_id 校验）兜底。子工作流内部保留环节结果存储节点（save_node_result），续跑按存储记录回放。
>
> **选择智能体语义直调的理由**：① 平台配置规范要求"调度中枢用智能体"（见《SitechAI开发平台配置规范.md》），独立调度主流程与规范职责重叠冗余；② 独立调度主流程存在真实缺陷（悬挂引用、未路由主业务链路），无法触达全链路；③ 「确认执行」后携带 req_id 的注入问题由子流自查链路（query_node_result 按 req_id 自查）在子流内部解决，无需主调度注入；④ 逐环节/防跳步/续跑等确定性纪律固化为提示词并由后端硬校验兜底，行为仍可审计、可测试。

**执行主干调度时序（智能体语义识别"确认配置"后逐环节推进，每环节暂停等用户反馈）：**

```
用户回复"确认执行"（携带上轮 req_id）
  ▼
① 智能体语义识别命中"确认配置"（req_id 沿用最近值；req_id 由 wf_sub_00/wf_sub_01 代码节点
    以系统时钟生成，PLAN+yyyyMMddHHmmss+3位随机数，LLM 不参与）
② 确认语义由智能体提示词【限制】识别（**无需写 CONFIRMED 确认标记**——后端
    save_product_config 已移除该门禁，识别命中即调度首个环节）
  ▼
③ 【环节1】调度 wf_sub_02 智能配置（入参 req_id；子流自查 requirement→提取执行方案原文→落地）
    出参 status/offer_id/save_result
    ├─ status==SUCCESS → 打印【环节1结果】→ 子流内部环节结果存储已写入
    │   （wf_sub_02 内置 save_node_result，node_name=config，req_id=入参）
    └─ 异常 → 打印异常节点+原因
    → **暂停，输出下一步建议，等待用户反馈**（成功提示【继续】，异常引导【重新执行】/【修改执行方案】）
  ▼ 用户回复【继续】
④ 【环节2】调度 wf_sub_03 规格稽核（入参 req_id，子流自查 config 取 offer_id/config_json）
    出参 pass/error_list/audit_summary
    ├─ pass==1 → 打印【环节2结果】→ 子流内部已存 node_name=spec
    └─ pass==0/接口异常 → 打印异常
    → **暂停，等待用户反馈**（同上）
  ▼ 用户回复【继续】
⑤ 【环节3】调度 wf_sub_05 资费校准（入参 req_id，子流自查 config 取 config_json）
    出参 pass/risk_list
    ├─ pass==1 → 打印【环节3结果】→ 子流内部已存 node_name=fee
    └─ 不通过 → 打印异常
    → **暂停，等待用户反馈**（同上）
  ▼ 用户回复【继续】
⑥ 【环节4】调度 wf_sub_04 自动测试（入参 req_id，子流自查 config 取 offer_id）
    出参 globalId/测试报告（含受理验证结论 orderId/offerInstId）
    ├─ test_passed==通过 → 打印【环节4结果】→ 子流内部已存 node_name=test
    └─ 失败/超时 → 打印异常
    → **暂停，等待用户反馈**（同上）
  ▼ 用户回复【继续】/四环节全部成功
    ⑦ 智能体汇总打印各环节成功详情 + 提示"是否发起上线审批"
    ………… 用户回复"发起审批" → ⑧ 直调 wf_sub_06（入参 req_id：
    子流串行自查 5 类环节结果 → 子流生成报告 → 报告存储(node_name=report)
    → submit_release_approval 推送审批）→ 输出审批单号
```

**调度要点（与 3.2 提示词一一对应）：**
1. **确认门禁**：后端 `save_product_config` 不校验 confirmed 入参与 CONFIRMED 标记——确认与否由智能体语义识别判断，防跳步职责回归提示词约定；`submit_release_approval` 仍校验 req_id 四环节结果齐全（执行主干未走完不得发起审批）。
2. **逐环节推进纪律**：严禁并行调用、跳过环节、连续调用多环节、凭语义推断环节成败（仅依据工具出参字段 status/pass/test_passed 判定）；每环节完成后必须暂停等待用户反馈，用户未回复前不得自行推进下一环节。
3. **结果存储**：wf_sub_02~05 各内置 save_node_result 节点（req_id=入参，node_name=config/spec/fee/test），子流执行成功即自动落库，LLM 无需再调存储工具；wf_sub_06 自查依赖此数据，存储缺失将致审批校验失败（fail-safe）。
4. **续跑**：`重新执行` 时按 fail_node 映射从失败环节续调（STAGE1_CONFIG→wf_sub_02、STAGE2_AUDIT→wf_sub_03、STAGE3_FEE→wf_sub_05、STAGE4_TEST→wf_sub_04），已成功环节不重复调用（按 req_id 自查存储回放）。
5. **消息查询**：审批进度/监控结果由智能体直接调用 `query_approval_status`、`query_product_monitor`（wf_sub_07/wf_sub_08 仅作平台不支持直调工具时的兜底）。
...
- 建议处理：可输入"继续"进入【下一环节名称】。{（末环节）或输入"发起审批"提交上线审批}
```
> 每环节成功输出后**暂停**，等待用户回复【继续】再调度下一环节；末环节成功后提示"是否发起上线审批"。
...
- 整改建议：{LLM 依据出参生成的建议}
请选择下一步：
① 回复【重新执行】：将从本失败环节继续（已成功环节不重复执行）
② 回复【修改执行方案】：请说明修改意见，将重新生成执行方案并再次确认
```
> 环节异常同样**暂停**，等待用户反馈后再行动。

**成功结果详情输出格式（四环节逐一推进全部成功后统一模板）：**
```
【执行主干全部完成】✅ 共4个环节执行成功：
...
### 6.2 子工作流拆分（11 个子工作流 JSON）

> 实现口径（与 `工作流配置/智能体工作流集V1.6/` 导出 JSON 一致，`gen_workflows_v2.py` 确定性生成）：**11 个子工作流 `wf_sub_00`~`wf_sub_10`（无独立意图调度主流程，由智能体按 3.2 提示词【意图→工作流映射表】语义识别直调）**。各子工作流统一 **req_id 单必填入参**（唯一批次标识，方案批次与执行主干共用；由 wf_sub_00 需求提报代码节点生成），子流程内部通过 query_node_result 自查所需上游结果，并在结束前通过**内置环节结果存储节点**（save_node_result，req_id=入参，node_name=本环节名）落库；确定性逻辑内嵌为 **type=6 代码节点**（CODE_*）。
...
> **实现状态说明**：上表 11 个子工作流 JSON 为当前实现基线（`工作流配置/智能体工作流集V1.6/`，`gen_workflows_v2.py` 生成；另有 2 份合并 export JSON 供平台联调参考）。以 11 子工作流 JSON + 智能体提示词语义调度 + knowledge/ 知识库 + 后端 `/api/v1/appstore/*` 适配端点为准。
>
...
**① 需求分析大模型节点（@deprecated：本小节为平台工作流历史口径，现行以 wf_sub_01 模板轨为准）**
- 温度值：0.2
- 输入：`requirement_text`（+ `query_similar_offer` 返回的相似产品列表）
- 链路：**环节1 需求理解与要素拆解**（仅提取要素：24 字段未提及项 value 填空字符串，不做完整性判断、不输出 pending_fields）→ **环节2 相似产品查询**（入参=要素摘要 need_summary）→ **环节3 产品信息整合**（匹配产品全量配置 + 要素部分信息 → 最终产品信息，source 仅"原始需求/AI补全"）→ **环节4 字段本体推理**（工具14 reason 一体推理）
- 补全规则：**仅价格类字段（套餐档位）未提取到 → "待补充"（禁止推理、禁止从相似产品照搬）**；套餐编码 → 不补全，值填"系统待生成"；其余缺失字段按取值链补全——相似产品 offerInfo 同构 24 字段（source"AI补全"）→ 本体推理引擎默认值（source"AI补全"）
...
**② 用户确认交互节点（由智能体提示词【限制】识别）**
- 子工作流 wf_sub_00/wf_sub_01 输出执行方案表格 + 提示语："请核对以上执行方案，回复【确认执行】将触发智能配置落地；如需调整请直接说明修改意见。"
- 用户确认语义识别：由智能体提示词【限制】识别确认词表"确认配置/确认执行/同意/可以/执行吧"等（未命中确认语义不触发执行主干）；确认类回复串行直调执行主干子流（无需写 CONFIRMED 确认标记、无需主工作流注入 confirmed 参数）。

**③ 智能配置节点（直读 JSON，不重新生成）**
- 节点链：开始节点(req_id) → **query_node_result（GET，req_id=开始节点入参，node_name=requirement）** → **代码节点提取 list[0].result_json 原文（CODE_EXTRACT_RECORD）** → `save_product_config`
...
- 确认语义：`save_product_config` **不设确认门禁**（不校验 confirmed 入参与 CONFIRMED 标记）——"未确认不配置"约束由智能体提示词【限制】的确认语义识别保证，后端只做幂等与 plan_json 合法性校验。

**④ 实时稽核节点（实时接口，无文件上传/无轮询）**
- `wf_sub_03` 子工作流：开始节点(req_id) → query_node_result 自查 config（按 req_id+node_name=config 读取环节结果记录）→ **代码节点提取 result_json 原文** → 单插件节点 `realtime_spec_audit`（入参 `offer_id + config_json`，**同步返回**稽核结果；offer_id/config_json 全部取自 config 环节结果）→ 大模型整改建议 → save_node_result(node_name=spec)。
- 判定：大模型节点根据 `pass`/`error_list` 提炼"通过/驳回 + 整改建议"；不通过走 `send_alert` + 终止分支。
- 原文件上传（audit_file_upload）、异步报告（audit_report_notify）、插入记录（audit_result_insert）、SQL 轮询（query_audit_by_sql）四个工具**不再使用**（保留在接口清单备查）。

**⑤ 智能测试节点组（发起动作，报告含受理验证）**
- `wf_sub_04` 子工作流：开始节点(req_id) → query_node_result 自查 config（读取 config 环节结果记录）→ **代码节点提取原文** → `offer_test`(offerId) **发起**（offerId 取自 config 环节结果）→ `get_test_scenes`(globalId) → **type=6 代码节点轮询 `get_test_progress`**（inputs 平铺 list；asyncio.sleep 间隔 5s、超时 30 分钟；done==true 退出；failed==true 提前退出并定位 failIndex）→ `get_test_result` → **CODE_MAP_FIXED_CASES 31 条固定用例映射** → 九章节正式版测试报告 → save_node_result(node_name=test)
...
- 测试报告模板：九章节正式版（测试流水/场景数与测点统计/逐场景明细/AI 总结与建议/**受理验证结论**（orderId、offerInstId、各受理场景是否通过）等）。

**⑥ 选择器节点（通用）**
- 条件模板：`{环节结果字段} 等于 通过值` → 走"通过"分支；否则走"驳回"分支。
- 支持且/或多条件（如 pass==1 且 failed==0；approve_confirmed==true）。

**⑦ 报告生成节点（位于 wf_sub_06 子工作流内部）**
- 位置：`wf_sub_06` 上线审批子工作流内。
...
**⑧ 逐环节结果打印与异常处置（由智能体提示词【限制】约束）**
- **环节结果打印**：每个环节子流返回后，由智能体按 6.1 节"每环节结果打印格式"模板输出该环节执行结果（引用出参原文，不凭语义推断成败）+ 下一步建议。
- **逐环节暂停**：每个环节输出后**必须暂停**，等待用户反馈——成功时提示回复【继续】再调度下一环节；用户未回复前不得自行推进（严禁连续调用多环节）。
- **异常处置**：环节异常时同样暂停，按"异常处置输出格式"模板输出异常环节名称、异常原因（引用接口返回原文，不臆测）、关键明细与整改建议，并引导【重新执行】/【修改执行方案】，不得自行重试。
- **续跑映射**：`重新执行` → 按 fail_node 续调（`STAGE1_CONFIG`→wf_sub_02、`STAGE2_AUDIT`→wf_sub_03、`STAGE3_FEE`→wf_sub_05、`STAGE4_TEST`→wf_sub_04）；已成功环节不重复调用（凭存储记录回放）。
- **环节结果存储**：wf_sub_02~05 各内置 save_node_result 节点（req_id=入参 req_id，node_name=config/spec/fee/test），子流执行成功即自动落库（后端 pd_ai_node_results 表持久化），调度层无需补存；wf_sub_06 审批自查依赖此数据，`submit_release_approval` 后端硬校验四环节（config/spec/fee/test）结果齐全。

**⑨ 审批发起确认（由智能体语义识别触发）**
- 四环节逐一推进全部成功后，智能体输出"成功结果详情 + 是否发起上线审批"提示，等待用户回复。
- 用户回复"发起审批/确认上线" → 智能体语义识别命中"发起审批"直调 `wf_sub_06`（入参 req_id=上下文沿用原值：子流串行自查 5 类环节结果 → 子流生成报告 → 报告存储 → 审批推送，**审批推送节点 req_id 引用开始节点**）→ 输出审批单号。
- 用户回复"暂不" → 提示可稍后发送"发起审批"继续（执行方案与各环节结果均以 req_id 同键存储，随时可续）。
- 兜底：`submit_release_approval` **后端硬校验** req_id 入参 + 四环节（config/spec/fee/test）结果齐全，跳过执行主干直接发起审批将被拒绝。
...
### 6.5 工作流 JSON 实现（当前实现基线）

> 本节为 6.1~6.3 节平台工作流描述的代码实现等价物（对应关系见 1.4 节）。当前实现基线与 `工作流配置/智能体工作流集V1.6/` 导出的 11 个子工作流 JSON 一致，由 `gen_workflows_v2.py` 确定性生成；业务逻辑与第 2~6 章平台工作流口径严格等价，仅是「实现载体」迁移——确定性逻辑化为工作流 type=6 代码节点 + 智能体提示词语义调度 + `knowledge/` 知识库 + 后端 `/api/v1/appstore/*` 适配端点。

#### 6.5.1 工作流 JSON 集结构（11 个子工作流）
...
> 说明：上表 11 个子工作流均为完整独立工作流（自带开始节点/req_id 入参/自查链路/结束节点），由智能体按 3.2 提示词【意图→工作流映射表】语义识别后直调。
...
意图路由规则与 3.2 节【意图→工作流调度映射表】等价：首次提报/修改需求→wf_sub_00→wf_sub_01；确认配置→先调度 wf_sub_02，此后按用户【继续】逐环节推进 wf_sub_03→wf_sub_05→wf_sub_04（每环节执行后暂停等反馈）；重新执行失败环节→按 fail_node 映射续调（STAGE1_CONFIG→wf_sub_02、STAGE2_AUDIT→wf_sub_03、STAGE3_FEE→wf_sub_05、STAGE4_TEST→wf_sub_04，req_id 沿用原值）；发起审批→wf_sub_06；审批通过自动上线+监控运维方案→wf_sub_06 尾段；审批进度查询→wf_sub_08；监控运维→wf_sub_07（异常分支根因闭环）；存量查询→wf_sub_09；存量合规扫描→wf_sub_10；业务问答→按 K1~K5 知识库检索；超范围→按答案为空提示回复。

调度纪律（固化于 3.2 提示词【限制】，后端硬校验兜底）：确认语义未命中不得进入执行主干；执行主干必须逐环节推进、严禁跳过环节、严禁连续调用多环节、严禁重复调用已成功环节（续跑时已成功环节按存储记录回放）；每环节完成后须暂停等用户反馈，仅依据后端出参字段判定成败（status/pass/test_passed），失败即暂停引导；审批后端四环节硬校验兜底。

#### 6.5.4 后端适配端点与代码节点通用行为
...
| **最佳实践：确认门禁取舍**（后端硬门禁改交 LLM 语义识别） | 后端已移除确认门禁，确认与否交智能体提示词【限制】语义识别（见 8.1/8.2）；**审批保留四环节硬校验**（submit_release_approval 校验 config/spec/fee/test 结果齐全）。 |
...
| 阶段6 验证与优化 | 以《产品信息.txt》销售品改写需求样例跑通全流程；验证"未确认不配置"约束（智能体确认语义识别）、智能体逐环节推进（每环节暂停等反馈）与结果打印、异常暂停引导与续跑、审批发起门禁（四环节硬校验）、消息查询（审批进度/监控结果）、测试报告受理验证结论完整性；提示词迭代 | 验证报告 | 4天 |
| 合计 | — | — | 约22个工作日 |
...
2. **工作流级**：
   - 正向用例：完整合规需求（取《产品信息.txt》中某销售品资费规则改写）→ 执行方案生成（节点结果存储保存）→ **用户确认（智能体识别确认语义，无需写标记）** → 智能体逐环节推进执行主干（确认后先调度 wf_sub_02，每环节打印结果+建议后**暂停**，用户回复【继续】再依次推进 03→05→04）→ 全部成功后打印成功详情并提示"是否发起审批" → 用户确认 → 审批单生成；
   - 反向用例：**未确认执行方案直接要求配置 → 智能体按提示词拒绝调度智能配置（确认语义识别）**；**主干全部成功后未确认直接要求审批 → 调度 wf_sub_06 后 submit_release_approval 因四环节结果校验拒绝/按提示词先引导确认**；构造资费冲突（如叠加优惠互斥）→ 资费校准拦截并暂停引导；构造配置缺陷 → 实时稽核驳回（pass=0）暂停引导；
   - 逐环节暂停用例：每环节成功输出后 → 验证智能体**暂停等待用户反馈**、不自行推进下一环节；用户回复【继续】→ 验证恰好推进一个环节（不多调、不跳调）；
   - 异常暂停用例：稽核不通过 → 验证智能体暂停、打印异常节点"配置规格稽核"+原因+建议、引导重新执行/修改执行方案；选择【重新执行】→ 验证从稽核环节续调且不重复落地；选择【修改执行方案】→ 验证回到需求分析；
   - 硬校验用例：跳过环节直接调 submit_release_approval（四环节结果不全）→ 校验拒绝；
   - 消息查询用例：审批推送后发送"查询审批进度"→ 返回审批单状态/当前环节；发送"查询监控结果"→ 返回监控指标与告警；
...
| 指标 | 目标 |
| --- | --- |
| 端到端流程贯通率 | 100%（给定需求→执行方案→确认→执行主干逐环节推进（配置落地→实时稽核→资费→测试报告含受理验证）→成功详情+审批确认→审批单生成） |
| 确认门禁有效性 | 未确认时配置落地触发率 = 0%（智能体确认语义识别）；未确认发起审批时审批推送触发率 = 0%（四环节结果校验） |
| 逐环节推进纪律 | 正常情况下每环节执行后 100% 暂停等待用户反馈，无自行推进、无并行调用、无跳步、无连续调用多环节；每环节结果打印率 100% |
| 异常处置完整性 | 异常暂停时 100% 输出异常节点名称、异常原因、整改建议，并给出重新执行/修改执行方案引导 |
| 续跑正确性 | 重新执行时已成功环节不重复执行，续跑起点与失败环节一致 |
| 消息查询可用性 | 审批进度查询、监控结果查询命中率 100%（意图可识别、结果可返回） |
...
| 幕2 用户确认 | 确认门禁有效性 | 先演示"未确认要求配置→拒绝"，再回复"确认执行"→ 调度首个环节（智能配置） |
| 幕3 执行主干逐环节推进 | 逐环节推进纪律 + 每环节结果打印 | 每环节执行后暂停并打印结果+建议，回复【继续】再推进下一环节（智能配置→稽核→资费→测试） |
| 幕3A 异常暂停与引导 | 异常处置完整性 + 续跑正确性 | 注入稽核缺陷 → 暂停并打印异常节点/原因/建议 → 演示【重新执行】续跑 |
| 幕3B 成功详情与审批确认 | 审批发起门禁 | 四环节成功详情打印 → 提示是否发起审批 → 确认后审批单生成 |
...
| 幕6-7 上线审批 | 端到端贯通 | 报告汇总（含受理验证）+ 审批单号输出 + **消息查询审批进度演示** |
| 幕8 监控运维 | 运维闭环 | 监控指标输出 + 可选 high 告警演示 + **消息查询监控结果演示** |
- 剧本附"反向分支速查表"，验收时按表逐项核验拦截能力（含"未确认不配置"一条），与 8.2 拦截率指标直接对应。

## 9. 上线与运维

1. **发布**：智能体发布到小思页面；工作流、插件保持版本可回退（平台支持保存草稿/发布双状态）。
2. **监控**：每日定时（或由运维对话触发）运行 `wf_sub_07` 监控子工作流，对 error_count 超阈值、fee_error_rate 异常的销售品调用 `send_alert` 告警；用户可随时发送"查询监控结果"消息触发即时查询。
3. **审批跟踪**：审批推送后，用户可随时发送"查询审批进度"消息调用 `query_approval_status` 查询审批状态；审批驳回时数字员工同步驳回原因并引导修改后重新发起。
4. **迭代机制**：
   - 知识库持续更新（新规范、新资费政策）；
...
| 用户未确认即触发生产写入 | 智能体提示词【技能2】+【限制2/3】确认语义识别约束（后端 save_product_config 已移除 CONFIRMED 门禁，"未确认不配置"由智能体语义识别保证） |
| 执行主干暂停后重复执行已完成写操作 | 各子工作流以 req_id（统一键）、node_name=config/spec/fee/test/report 落库节点结果存储（后端持久化）；续跑时子工作流自查回放已成功环节，写接口不重复调用 |
| 未经确认发起审批 | 后端 submit_release_approval 硬校验 req_id 四环节（config/spec/fee/test）结果齐全 + 智能体提示词【技能3】【限制6】审批引导约束 |
| 消息查询意图未命中 | 智能体提示词【技能4】明确"审批进度/审批状态/监控/运行监控"触发词；查询工具入参缺失时提示用户补齐 offer_id/approval_id |
...
| 流程环节被跳过 | 双保险：智能体提示词逐环节推进纪律（【限制4】严禁跳过环节、严禁连续调用多环节、严禁凭语义推断成败）+ submit_release_approval 四环节结果硬校验（审批环节跳步即被拒绝）；配置落地环节依赖确认语义识别 |
...
- [x] 智能体按 3.2 提示词【意图→工作流映射表】语义识别直调（需求提报/确认配置/失败续跑/上线审批/审批进度/监控运维/存量查询/存量合规/QNA 等），确定性纪律由【限制】+ 后端硬校验兜底
...
- [x] 主链路端点连通（2026-09-19 实跑验证）：result/save、result/query、audit/realtime、product/config/save、billing/rules/verify、test/offer/start、test/offer/result、approval/submit、product/monitor、similar/offer/query 已 live 生效
- [ ] 14 个既有契约端点＋7 个适配端点逐一连通后端（含错误码 PARAM_MISSING/5002/ONTOLOGY_EMPTY 验证）；仍占位：/report/download、/script/download（本轮实跑 backend_pending=1 回退）；/ops/root-cause、/ops/work-orders、/shelf-compliance、/validate-nested、/explain 待逐项确认
- [ ] 端到端联调：正向全流程 + 反向用例（未确认不配置、跳步审批被拒、每环节暂停等反馈、稽核驳回暂停引导、续跑不重复写）+ 18 销售品兼容回归（对齐 8.1/8.2 节）
  > **2026-09-19 正向前向链路已真跑**（8 子工作流串行，零回退零错误）：req_id=PLAN20260919182810908 / offer_id=982810908 / globalId=50202609191828404195108786 / 上线审批单 AP202609191829100019（通过）。待整改差异点：渠道 APP 未显性落地、串行顺序（测试在资费前）致受理场景未覆盖、test/offer/scenes 未调用（日志见 `工作流配置/wf_runner/logs/`，差异点汇总见《端到端演示剧本》1.5.4）。
