...
| V2.1 | 2026-09-13 | **字段补全改为本体推理引擎实现（K6 知识库文档方案整体替换）**：删除 K6 字段本体库知识分类（知识库恢复 K1~K5 五分类）；四类18字段的枚举/格式/默认值/兜底口径改由**后端字段本体推理引擎**（FieldOntologyService，工具14 field_ontology_reason，POST /api/v1/appstore/ontology/fields，action=validate/complete/ontology）以代码为单一事实源提供；wf_sub_01 新增「字段本体校验」节点（工具14 action=validate）对 LLM 补全结果逐字段推理校验，非法值返回 violations 及期望规则提示重填；缺失字段由引擎按本体默认值推理补全（兜底口径字段不默认补全）；插件集 13→14 个工具 |
...
- **取值链与本体推理引擎（V2.1，字段本体由后端推理引擎统一保证）**：
  ...
  （GB/分钟/条/元）。LLM 补全后由 wf_sub_01 字段本体校验节点（工具14 action=validate）
  逐字段推理校验，非法值返回 violations 及期望规则提示重填；缺失字段由引擎按
  本体默认值补全（action=complete），兜底口径字段（套餐固定费/三类资源）不默认补全。
- **待补充触发条件（V1.9，仅两种，满足其一才判"待补充"）**：
  ① 套餐固定费（月租费）未提取到；
  ② 流量/语音/短信三类资源一个都未提取到（此时未提取到的资源字段全部填
  "待补充"；任一类已提取到则其余资源字段不算缺失，可AI补全）。
  待补充字段禁止推理，不得从相似产品照搬；列入 pending_fields，
  提示用户在确认执行方案时补充。
