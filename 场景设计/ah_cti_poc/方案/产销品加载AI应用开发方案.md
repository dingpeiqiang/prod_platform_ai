...
| --- | --- | --- | --- | --- | --- |
| 需求分析（执行方案生成） | `wf_sub_01` | 9/8 | requirement_text, requirement_file | req_id(存储key) / plan_md（执行方案表格）/ plan_json | 开始 → 大模型(需求理解与要素拆解，仅提取要素信息) → query_similar_offer(相似产品，入参=要素摘要) → 大模型(产品信息整合：匹配产品全量配置+要素部分信息→最终产品信息，单出参 fields_output 仅字段数组) → 字段本体推理(工具14 reason 一体推理) → 代码节点004a(单入参=推理后字段数组：系统生成 req_id 并组装 plan_json，plan_md 代码重新生成四列表格，pending_fields 反查待补充) → 选择器(待补充项提示) → 节点结果存储·结果存储(保存JSON) → 结束 |
