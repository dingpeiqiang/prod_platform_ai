...
**需求提报（【环节1/8】）→ 需求分析（【环节2/8】，五列模块表格《加载方案》）→ 用户确认 → 【智能配置（【环节3/8】配置落地）→ 规格稽核（【环节4/8】）→ 资费校准（【环节5/8】）→ 自动测试（【环节6/8】，含受理验证子集）】自动串行（确认后一次跑完四环节，仅失败中断）→ 上线审批（【环节7/8】上线校验看板，用户二次确认后发起）→ 监控运维（【环节8/8】确认上线后生成监控运维方案）** 的端到端闭环。
...
| 平台能力 | 在本场景中的角色 |
| --- | --- |
| 智能体（助手） | 产销品数字员工统一入口（对话式 + 流程式调度中枢）；由智能体（LLM）按 3.2 提示词【意图→工作流映射表】**语义识别直调** 12 个子工作流（确认后自动串行跑完四环节、每环节打印结果、仅失败中断与异常引导由提示词【限制】约束），后端硬校验兜底；**需求工单审批须用户明确确认后才发起**（提报单就绪止于确认点，回复【发起需求审批】后由 `wf_sub_11` 发起）；流程结束后承接**消息查询**（审批进度/监控结果）与**异常处置引导**（重新执行/修改执行方案） |
...
### 1.3 设计原则
1. **子工作流化 + 智能体语义调度**：每个环节封装为子工作流（`wf_sub_00`~`wf_sub_11`，共 12 个；req_id 单必填入参自查链路）；无独立意图调度主流程，由智能体（LLM）按 3.2 提示词【意图→工作流映射表】语义识别后**直调**各子流，确定性纪律（确认门禁、串行跑完不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败）固化在提示词【限制】并由后端硬校验兜底。
...
8. **自动串行，仅失败中断**：用户确认后的执行主干（智能配置→实时稽核→资费校准→自动测试）按固定顺序**一次串行跑完、中途不停顿**，每完成一个环节即打印该环节处理结果并**自动进入下一环节**（无需用户回复）；**仅环节失败/异常时中断**，打印异常节点+异常原因，并引导用户【重新执行】或【修改执行方案】；全部成功后打印各环节成功详情并提示"是否发起上线审批"。
9. **审批二次确认（双轨）**：**需求工单审批**不自动触发，须在需求提报单生成后由用户明确回复【发起需求审批】才发起（`wf_sub_00` 提报单就绪即止于确认点，`wf_sub_11` 承接发起）；**上线审批**不自动触发，须在执行主干全部成功后由用户确认发起。
10. **人机协同**：用户确认执行方案、确认发起需求工单审批、确认发起上线审批三个决策点保留人工确认；**执行主干确认后自动串行跑完四环节（仅失败中断）**；流程结束后支持消息查询审批进度与监控运维结果。
...
| **执行主干自动串行** | 智能配置→稽核→资费→测试按序推进，**确认后一次跑完四环节、中途不停**；每环节打印结果+下一步建议，异常打印节点与原因并引导重新执行/修改执行方案 | 智能体语义识别"确认配置"后**一次串行调度四环节** `wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`（中途不停，仅失败中断）；异常按 fail_node 续跑（门禁由后端硬校验） |
...
> 本节提示词为**智能体意图识别 + 子工作流直调**的权威定义：智能体（LLM）按下方【意图→工作流映射表】语义识别后**直调各子工作流**（`wf_sub_00`~`wf_sub_11`，共 12 个子流；子流均自带开始节点/req_id 入参/自查链路/结束节点，为完整独立工作流）。**全流程按 8 环节口径**（受理验证并入环节6，不独立成节）。执行主干为**自动串行模式（参考 flow-B 口径）**：用户回复确认类语句后**一次跑完四环节（智能配置→配置规格稽核→资费校准→自动测试）中途不停**，各环节成功后**立即打印该环节结果**并自动进入下一环节（无需用户回复），仅环节失败时中断。**需求分析生成《加载方案》后须用户明确确认才可继续智能配置**（`wf_sub_01` 输出方案即止于确认点，未回复确认前严禁调度 `wf_sub_02`）。**需求工单审批须用户明确确认后才发起**（`wf_sub_00` 提报单就绪即止于确认点，用户回复【发起需求审批】后由 `wf_sub_11` 发起）。确定性纪律（确认门禁、串行不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败、审批四环节后端硬校验兜底）固化为下方【限制】。提示词按【角色 + 技能 + 限制】三段式组织。
...
【技能】
1. 需求提报与分析（【环节1/8】+【环节2/8】）：理解需求→拆解要素（3 模块/9 分类/24 字段）→补全→生成《加载方案》
   （五列模块表格 + 执行方案 JSON，经节点结果存储保存，返回 req_id）。需求原文最高
   优先、不得虚构；来源仅"原始需求/AI补全"两态；仅套餐档位（价格）缺失且无参照才
   "待补充"（禁推理价格）；套餐编码填"系统待生成"；有待补充则列出并提示补充。
   **需求提报单生成后必须提示"是否发起【需求工单审批】"并止于确认点**（`wf_sub_00`
   渲染提报单后不发起审批），用户明确回复【发起需求审批】后才调度 `wf_sub_11` 发起
   `submit_release_approval`(approval-type=requirement)；未表态则停留等待，禁止擅自发起。
   **执行方案生成后同样止于确认点**（`wf_sub_01` 输出五列模块表格 + 执行方案 JSON 后
   必须提示"请确认执行方案，回复【确认配置】后开始配置落地"并**停止**），**用户明确回复
   确认类语句（确认配置/确认执行/同意/可以/执行吧）前，严禁调度 `wf_sub_02`、严禁进入
   执行主干**；未确认则停留等待，禁止擅自配置（详见【限制】第2/3/5 条）。
2. 执行主干（【环节3/8】~【环节6/8】，确认后自动串行四环节，中途不停）：识别确认语义
   （确认配置/确认执行/同意/可以/执行吧）→取最近 req_id→**一次跑完四环节，中途不停**，
   顺序为 `wf_sub_02` 智能配置（环节3）→ `wf_sub_03` 规格稽核（环节4）→ `wf_sub_05`
   资费校准（环节5）→ `wf_sub_04` 自动测试（环节6，受理验证为其子集，随测试结果输出）。
   **每环节执行后立即打印该环节结果**（引用出参原文，不凭语义推断）**并自动进入下一
   环节，无需用户回复**；每环节成功后按"每环节结果输出模板"输出结果 + 固定引导行
   （"建议处理：无需操作，将自动进入【下一环节】（无需回复，串行自动推进；如需中止可
   回复【修改执行方案】）"）。环节结果由子流 save_node_result 落库
   （node_name=config/spec/fee/test）。
   环节成功（status==SUCCESS/PARTIAL / pass==1 / test_passed==通过）→自动续跑下一环节；
   环节异常（status≠SUCCESS 等）→**中断**，打印异常环节+原因+关键明细，引导【重新执行】
   （按 fail_node 续调，已成功环节不重复执行）或【修改执行方案】。
   **仅环节失败/异常时中断**，用户确认后无需任何额外回复即可跑完全部四环节。
3. 审批与上线（【环节7/8】）：四环节全部成功后输出【执行主干全部完成】汇总块（每环节
   一行：环节/执行结果/关键数据）并提示"是否发起上线审批"；用户确认后调度 `wf_sub_06`
   （自查四环节+报告推送，双轨）；审批通过自动上线并生成监控运维方案（通过前严禁生成）。
4. 查询与运维（【环节8/8】+辅助）：按消息直调 `wf_sub_08` 审批进度（双轨：需求工单审批/
   上线审批）、`wf_sub_07` 监控运维（offer_id，异常分支根因推理+建工单）、`wf_sub_09`
   存量查询（只读）、`wf_sub_10` 存量合规扫描。

【意图→工作流映射表（语义识别，直调子流）】
| 用户意图 | 调度子流/动作 | 关键入参 |
| --- | --- | --- |
| 提报/修改需求（【环节1/8】） | `wf_sub_00`（渲染提报单，**止于确认点**）→ 用户回复【发起需求审批】后 `wf_sub_11` 发起需求工单审批 → 审批通过后 `wf_sub_01` | requirement_text；req_id 沿用 |
| 发起需求工单审批（【环节1/8】） | `wf_sub_11`（须用户明确确认后才发起） | req_id=最近值 |
| 生成加载方案（【环节2/8】） | `wf_sub_01`（输出五列表格+执行方案 JSON，**止于确认点**：未确认前严禁调度 `wf_sub_02`） | req_id=最近值 |
| 确认配置（【环节3/8】~【环节6/8】） | **一次串行跑完四环节**：`wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`（中途不停，仅失败中断） | req_id=最近值 |
| 重新执行失败环节 | 按 fail_node 续调（STAGE1→02/2→03/3→05/4→04），已成功环节凭存储回放不重跑 | req_id 沿用；自查回放 |
| 上线审批（【环节7/8】） | `wf_sub_06`（双轨） | req_id=原值 |
| 查询审批进度（辅助） | `wf_sub_08`（双轨：需求工单审批/上线审批） | approval_id/offer_id |
| 查询监控运维（【环节8/8】） | `wf_sub_07`（异常根因+工单闭环） | offer_id |
| 查询存量产品（辅助） | `wf_sub_09`（只读） | 产品名/ID |
| 存量合规扫描（辅助） | `wf_sub_10` | 存量范围 |
| 业务问答/超范围 | K1~K5 知识库检索 / 答案为空提示 | 检索词 |

【限制】
1. 仅回答产销品加载相关业务，超范围按答案为空提示。
2. **执行方案确认门禁**：需求分析（`wf_sub_01`）生成《加载方案》后**必须止于确认点**，
   输出五列模块表格 + 执行方案 JSON 并提示"请确认执行方案，回复【确认配置】后开始配置
   落地"，随即**停止等待用户回复**；**用户未明确回复确认前，严禁调度 `wf_sub_02`、严禁
   进入执行主干**。智能配置必须用户确认《加载方案》后触发，配置以节点结果存储保存版本为准。
3. 确认语义未命中绝不调度执行主干；req_id 沿用最近值，禁止重新生成。
4. **需求工单审批须用户明确确认后才发起**：`wf_sub_00` 提报单就绪即止于确认点，
   必须提示"是否发起【需求工单审批】"；用户未明确回复【发起需求审批】前，严禁调度
   `wf_sub_11`、严禁发起审批。
5. 执行主干必须按 `wf_sub_02`→`03`→`05`→`04` 顺序**串行自动跑完**，**中途不停顿、
   不等待用户回复、严禁并行、严禁跳步，严禁凭语义推断成败**（仅依据出参
   status/pass/valid/test_passed/backend_pending 判定）；**仅环节失败/异常时中断**。
6. 每环节执行后必须立即输出该环节结果 + 固定引导行（"建议处理：无需操作，将自动进入
   【下一环节】…"），随即自动续跑下一环节；已成功环节续跑不重复调用（凭存储回放）。
7. **异常（仅环节失败/异常）时中断**，按异常统一模板输出环节/原因/明细/建议，并引导
   回复【重新执行】（从本失败环节继续，已成功环节不重复执行）或【修改执行方案】，
   不得自行重试，也不得输出任何汇总表格。
8. 审批必须四环节全成且用户确认后才发起；审批通过前严禁生成监控运维方案。
9. 输出结构化：环节名/结果/关键数据/建议处理；✅ 与统计值须与出参一一对应，
   禁止凑数、虚构，不泄露资费配置明细；四环节全部成功后输出【执行主干全部完成】汇总块。
```
...
> **核心口径**：无独立意图调度主流程，调度职责回归智能体（LLM）——按 3.2 节提示词【意图→工作流映射表】语义识别后**直调** 12 个子工作流 `wf_sub_00`~`wf_sub_11`。常驻提示词精简（角色/技能/映射表/限制），自动串行、结果打印、异常引导由【限制】约束；确定性纪律（确认门禁、串行跑完不跳步、fail_node 续跑、req_id 沿用、仅依据出参判成败）固化为提示词硬性原则，并由后端硬校验（四环节门禁/req_id 校验）兜底。子工作流内部保留环节结果存储节点（save_node_result），续跑按存储记录回放。**全流程按 8 环节口径**（受理验证并入环节6，不独立成节），标题头统一 `【环节N/8·全名】`。
>
> **选择智能体语义直调的理由**：① 平台配置规范要求"调度中枢用智能体"（见《SitechAI开发平台配置规范.md》），独立调度主流程与规范职责重叠冗余；② 独立调度主流程存在真实缺陷（悬挂引用、未路由主业务链路），无法触达全链路；③ 「确认执行」后携带 req_id 的注入问题由子流自查链路（query_node_result 按 req_id 自查）在子流内部解决，无需主调度注入；④ 串行/防跳步/续跑等确定性纪律固化为提示词并由后端硬校验兜底，行为仍可审计、可测试。

**执行主干调度时序（智能体语义识别"确认配置"后一次串行跑完四环节，仅失败中断）：**
...
**调度要点（与 3.2 提示词一一对应）：**
1. **确认门禁**：**需求分析生成《加载方案》后须止于确认点**——`wf_sub_01` 输出方案即停止等待，未命中确认语义前智能体不调度 `wf_sub_02`。后端 `save_product_config` 不校验 confirmed 入参与 CONFIRMED 标记——确认与否由智能体语义识别判断，防跳步职责回归提示词约定；`submit_release_approval` 仍校验 req_id 四环节结果齐全（执行主干未走完不得发起审批）。
2. **自动串行纪律**：确认后一次跑完四环节（02→03→05→04），严禁并行调用、跳过环节、中途停顿等待用户回复、凭语义推断环节成败（仅依据工具出参字段 status/pass/test_passed 判定）；**仅环节失败/异常时中断**。
...
**每环节结果打印格式（LLM 输出统一模板，N 取值 3~6）：**
```
【环节N/8·{环节名称}】✅ 执行成功
- 关键数据：{该环节关键输出（如 offer_id/save_result、audit_summary、
  risk_list 为空、测试统计+受理验证小节 orderId/offerInstId）}
- 建议处理：无需操作，将自动进入【下一环节名称】。（无需回复，串行自动推进；
  如需中止可回复"修改执行方案"。末环节改为输入"发起审批"提交上线审批）
```
> 每环节成功输出后**自动进入下一环节**，无需用户回复；**仅环节失败/异常时中断**，等待用户反馈后再行动。末环节成功后提示"是否发起上线审批"。
...
```
> 环节异常即**中断**（串行自动推进中止），等待用户反馈后再行动。

**成功结果详情输出格式（四环节自动串行全部成功后统一模板）：**
```
【执行主干全部完成】✅ 共4个环节执行成功：
...
> 实现口径（与 `工作流配置/智能体工作流集V1.6/` 导出 JSON 一致，`gen_workflows_v2.py` 确定性生成）：**12 个子工作流 `wf_sub_00`~`wf_sub_11`（无独立意图调度主流程，由智能体按 3.2 提示词【意图→工作流映射表】语义识别直调）**。各子工作流统一 **req_id 单必填入参**（唯一批次标识，方案批次与执行主干共用；由 wf_sub_00 需求提报代码节点生成），子流程内部通过 query_node_result 自查所需上游结果，并在结束前通过**内置环节结果存储节点**（save_node_result，req_id=入参，node_name=本环节名）落库；确定性逻辑内嵌为 **type=6 代码节点**（CODE_*）。全流程按 **8 环节**口径（受理验证并入环节6，不独立成节）。
...
| 智能配置（【环节3/8】配置落地） | `wf_sub_02` | req_id（必填） | offer_id / save_result（含可选 group） | 开始(req_id) → query_node_result(自查 requirement) → CODE_EXTRACT_RECORD(取 result_json 原文) → save_product_config(配置落地) → **CODE_FUSION_GROUP_ECHO 融合组成员回显**（含可选 group/role/offer_id 成员集）→ **save_node_result(node_name=config)** → 结束 |
| 规格稽核（【环节4/8】，组维度） | `wf_sub_03` | req_id（必填） | pass / error_list / audit_summary | 开始(req_id) → 自查 config（取 offer_id/config_json）→ CODE_EXTRACT_RECORD → realtime_spec_audit(实时稽核, **组维度稽核**, error_list 含 group:role 定位) → 大模型(整改建议生成) → **save_node_result(node_name=spec)** → 结束 |
| 资费校准（【环节5/8】，成员分组） | `wf_sub_05` | req_id（必填） | pass / risk_list | 开始(req_id) → 自查 config（取 config_json）→ CODE_EXTRACT_RECORD → check_billing_rule(check_scene=all，**按 member_role 分组比对，E27 负值比对**) → 大模型(风险解读) → **save_node_result(node_name=fee)** → 结束 |
...
- **自动串行推进**：环节成功后**自动进入下一环节**（无需用户回复）；**仅环节失败/异常时中断**（确认后一次跑完 02→03→05→04，中途不停顿）。
...
**⑨ 审批发起确认（由智能体语义识别触发）**
- 四环节自动串行全部成功后，智能体输出"成功结果详情 + 是否发起上线审批"提示，等待用户回复。
...
意图路由规则与 3.2 节【意图→工作流调度映射表】等价：首次提报/修改需求→wf_sub_00（**止于确认点**）；发起需求工单审批→wf_sub_11（须用户明确确认）→审批通过后 wf_sub_01（**方案生成后止于确认点**）；确认配置→**一次串行跑完四环节** wf_sub_02→wf_sub_03→wf_sub_05→wf_sub_04（中途不停，仅失败中断）；重新执行失败环节→按 fail_node 映射续调（STAGE1_CONFIG→wf_sub_02、STAGE2_AUDIT→wf_sub_03、STAGE3_FEE→wf_sub_05、STAGE4_TEST→wf_sub_04，req_id 沿用原值）；发起审批→wf_sub_06；审批通过自动上线+监控运维方案→wf_sub_06 尾段；审批进度查询→wf_sub_08；监控运维→wf_sub_07（异常分支根因闭环）；存量查询→wf_sub_09；存量合规扫描→wf_sub_10；业务问答→按 K1~K5 知识库检索；超范围→按答案为空提示回复。

调度纪律（固化于 3.2 提示词【限制】，后端硬校验兜底）：**执行方案未确认不得进入执行主干**（wf_sub_01 方案就绪止于确认点，未回复确认不调度 wf_sub_02）；确认语义未命中不得进入执行主干；**需求工单审批须用户明确确认后才发起**（wf_sub_00 提报单就绪止于确认点，未回复【发起需求审批】前严禁调度 wf_sub_11）；执行主干必须串行自动跑完（02→03→05→04，中途不停顿）、严禁跳过环节、严禁并行调用、严禁中途停顿等待用户回复、严禁重复调用已成功环节（续跑时已成功环节按存储记录回放）；仅依据后端出参字段判定成败（status/pass/test_passed），失败即中断引导；审批后端四环节硬校验兜底。
...
| 阶段6 验证与优化 | 以《产品信息.txt》销售品改写需求样例跑通全流程；验证**执行方案确认门禁**（《加载方案》生成后止于确认点，回复确认前不调度 wf_sub_02）、"未确认不配置"约束（智能体确认语义识别）、**需求工单审批须用户确认后发起**（提报单就绪止于确认点，回复【发起需求审批】后才调度 wf_sub_11）、智能体自动串行推进（确认后一次跑完四环节、仅失败中断）与结果打印、异常中断引导与续跑、审批发起门禁（四环节硬校验）、消息查询（审批进度/监控结果）、测试报告受理验证结论完整性；提示词迭代 | 验证报告 | 4天 |
| 合计 | — | — | 约22个工作日 |
...
2. **工作流级**：
   - 正向用例：完整合规需求（取《产品信息.txt》中某销售品资费规则改写）→ **需求提报单生成并止于确认点 → 用户回复【发起需求审批】→ wf_sub_11 发起需求工单审批（approval-type=requirement）** → 执行方案生成（节点结果存储保存）→ **用户确认（智能体识别确认语义，无需写标记）** → 智能体自动串行推进执行主干（确认后**一次跑完四环节** wf_sub_02→03→05→04，每环节打印结果+建议后**自动进入下一环节**，无需回复）→ 全部成功后打印成功详情并提示"是否发起审批" → 用户确认 → 审批单生成；
   ...
   - 自动串行用例：确认后 → 验证智能体**一次跑完四环节、中途不停顿、不等待用户回复**；每环节成功输出后自动进入下一环节（不跳环节、不重复调用）；
...
| 确认门禁有效性 | 未确认时配置落地触发率 = 0%（智能体确认语义识别）；未确认发起审批时审批推送触发率 = 0%（四环节结果校验）；**需求工单审批未确认时发起率 = 0%（提报单就绪止于确认点，未回复【发起需求审批】不调度 wf_sub_11）**；**执行方案未确认时智能配置触发率 = 0%（方案就绪止于确认点，未回复确认不调度 wf_sub_02）** |
| 自动串行纪律 | 正常情况下确认后 100% 一次跑完四环节、中途不停顿、不等待用户回复，无并行调用、无跳步、无重复调用；每环节结果打印率 100% |
...
| 幕2 用户确认 | 确认门禁有效性 | 先演示"执行方案未确认直接要求配置→拒绝"（方案就绪止于确认点，不调度 wf_sub_02），再回复"确认执行"→ 一次串行触发执行主干四环节 |
| 幕3 执行主干自动串行 | 自动串行纪律 + 每环节结果打印 | 回复"确认执行"后一次跑完四环节并逐环节打印结果+建议（智能配置→稽核→资费→测试），中途不停 |
...
| 流程环节被跳过 | 双保险：智能体提示词自动串行纪律（【限制5】严禁跳过环节、严禁中途停顿等待用户回复、严禁并行调用、严禁凭语义推断成败）+ submit_release_approval 四环节结果硬校验（审批环节跳步即被拒绝）；配置落地环节依赖确认语义识别；需求工单审批依赖提报单确认点约束（未回复【发起需求审批】不调度 wf_sub_11） |
...
- [ ] 端到端联调：正向全流程 + 反向用例（**执行方案未确认不配置**、需求审批未确认不发起、未确认不配置、跳步审批被拒、确认后自动串行跑完四环节、稽核驳回中断引导、续跑不重复写）+ 18 销售品兼容回归（对齐 8.1/8.2 节）
