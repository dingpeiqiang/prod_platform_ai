...
> 版本：V3.3　日期：2026-09-19

> V3.7 履历 | 2026-09-20 | **智能体提示词职责分层重构（剥离技术编码逻辑，工作流 JSON 与后端零改动）**：① 3.2 提示词原【角色+技能+限制】混入了大量技术实现细节（`save_node_result`/`node_name=config,spec,fee,test`、`wf_sub_02`→`03`→`05`→`04` 硬编码顺序、成败判定编码 status/pass/test_passed/backend_pending、fail_node 续调映射、req_id 沿用/禁止重新生成、存储回放），并要求 LLM 负责这些确定性编排——**本版本将其全部从提示词剥离**，明确**职责分层**：智能体（LLM）只做 语义识别 / 业务理解与补全 / 结果表达 三件事；req_id 生成与透传、环节落库、成败判定、fail_node 续跑、审批四环节硬校验等确定性逻辑由**子工作流自查链路（query_node_result）+ type=6 代码节点 + 后端适配端点硬校验**兜底（权威描述保留在 6.1/6.2、4.4 端点契约、1.4 实现方式，本节不重复）；② 3.2 提示词改为【角色+技能+意图映射表+限制】四段式，映射表按"用户意图→业务动作→直调子工作流"重构，限制保留确认门禁/逐环节确认推进/结果表达铁律/职责不越界4 类约束；③ 同步校正 1.3 设计原则-1、1.2 智能体行、6.1 核心口径与"选择智能体语义直调的理由"④、调度纪律、附录 Checklist 中"确定性纪律固化为提示词【限制】"的表述，改为"由子流自查+代码节点+后端硬校验兜底，不依赖 LLM 自觉"；④ 业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约一律不变，工作流 JSON 与后端零改动
> V3.4 履历 | 2026-09-19 | **结束节点页面地址收敛为单面板单 url（对齐参考示例）+ JSON 紧凑无空格**：各子工作流结束节点末尾 `{panel}` 片段由「配置工作台(view=workbench) + 环节业务页(view=stage)」双面板收敛为**单面板、单 url**——url 统一用环节业务页 `config-workbench.html?offer_id=..&name=..&chatId=<实值>&stage=<N>&view=stage`（`title` 用环节业务名），移除 `product-detail.html` 单品运营看板外链与 view=workbench 冗余面板；同时 panel 代码节点输出 **JSON 紧凑无空格**（冒号/逗号后无空格，逐字对齐 `{"version":"1.0","message_id":"...","panels":[{"panel":"right","mode":"external","url":"...","title":"..."}]}`）；需求/方案阶段（00/01）`offer_id` 仍取 `req_id` 兜底。生成器 `gen_workflows_v2.py` + 11 个 JSON 同步（详见《工作流JSON开发规范.md》§十一单面板+紧凑JSON口径）
> 实现基线（V2.0 重塑起）：**13 个工作流 JSON（`wf_sub_00`~`wf_sub_11`，位于 `场景设计/ah_cti_poc/工作流配置/智能体工作流集V1.6/`，由 `gen_workflows_v2.py` 生成，无意图调度主流程，由智能体语义识别直调）+ `knowledge/` 知识库 + 后端 `/api/v1/appstore/*` 适配端点**；第 2~6 章平台工作流描述为业务口径与契约权威定义（见 1.4 节实现方式说明）

> V3.3 履历 | 2026-09-19 | **工作流调用顺序对照历史 skill 版本优化 + 需求工单审批拆分为独立子流 + 执行方案确认门禁强化**：① **需求工单审批须用户明确确认后才发起**——`wf_sub_00` 提报单就绪即**止于确认点**（渲染提报单 + "是否发起【需求工单审批】？"，不再直接串行发起审批），用户回复【发起需求审批】后由**新增子流 `wf_sub_11_发起需求审批`** 承接（自查 requirement_report → 提取原文 → `submit_release_approval`(approval-type=requirement) → 回执渲染）；② **需求分析生成《加载方案》后须用户明确确认才可继续智能配置**——`wf_sub_01` 输出五列模块表格+执行方案 JSON 即**止于确认点**，未回复确认类语句前严禁调度 `wf_sub_02`、严禁进入执行主干（文档/提示词强化，工作流 JSON 不改）；③ **子流集由 11 个扩为 12 个**（`wf_sub_00`~`wf_sub_11`，工作流 JSON 由 12 个增为 13 个），3.2 意图映射表新增"发起需求审批"/"生成加载方案"行；④ **全流程统一 8 环节口径**（1 需求提报 / 2 需求分析 / 3 智能配置 / 4 规格稽核 / 5 资费校准 / 6 自动测试 / 7 上线审批 / 8 监控运维；**受理验证并入环节6，不独立成节**），全文标题头统一为 `【环节N/8·全名】`，新增 8 环节总表；⑤ 全文 1.1/1.2/1.3/1.4/2 章/3.2/3.3/6 章/7~10 章/附录 Checklist 与版本履历同步；⑥ 业务口径/字段体系/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约一律不变（本次新增 `wf_sub_11` 与 `wf_sub_00` 拆分，其余子流编号不变）
> V3.2 履历 | 2026-09-19 | **执行主干环节调用逻辑对照历史 skill 版本（flow-B）优化——由"逐环节等反馈"回归"确认后自动串行"（提示词/文档同步优化，工作流 JSON 与后端零改动）**：① 智能体提示词执行主干纪律重写——用户确认配置后**一次串行跑完四环节（`wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`），中途不停顿**，各环节成功后**立即打印该环节结果**并自动进入下一环节（无需用户回复），**仅环节失败/异常时中断**；② 意图映射表"确认配置"行恢复为"一次串行跑完四环节"（删除"继续下一步"行）；③ 限制条款改写为串行纪律（串行跑完不跳步、不等待用户回复、仅依据出参判定、仅失败中断）；④ 1.1 建设目标、1.2 智能体行、1.3 设计原则、2 章能力映射、6.1 调度模式（含时序图自动串行）、6.3⑧⑨、6.5.3、7/8/9/10 章与附录 Checklist 同步；⑤ 业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约一律不变 |
...
| 版本 | 日期 | 变更说明 |
| --- | --- | --- |
| V3.7（智能体提示词职责分层重构） | 2026-09-20 | **剥离 3.2 提示词中的技术编码逻辑，确立智能体职责分层**：① 3.2 提示词移除 save_node_result/node_name、wf_sub_02→03→05→04 硬编码顺序、成败判定编码（status/pass/test_passed/backend_pending）、fail_node 续调映射、req_id 沿用管理等确定性编排细节，明确智能体只做 语义识别/业务理解/结果表达，上述确定性逻辑由子流自查链路+代码节点+后端硬校验兜底（权威描述保留在 6.1/6.2、4.4、1.4）；② 3.2 改为【角色+技能+意图映射表+限制】四段式，映射表按"意图→业务动作→子工作流"重构，限制保留确认门禁/逐环节确认推进/结果表达铁律/职责不越界；③ 同步校正 1.3-1、1.2、6.1 核心口径与理由④、调度纪律、附录 Checklist 的"确定性纪律固化为提示词【限制】"表述；④ 业务口径/字段体系/子流编号/fail_node 映射/req_id/落库 node_name/审批硬校验契约不变（工作流 JSON 与后端零改动，详见顶部 V3.7 履历） |
| V3.6（步进式人机协同 + 下一步建议） | 2026-09-20 | **全流程改为「每环节处理完输出结果+下一步建议、用户反馈后进入下一环节」步进式**：① 取消执行主干「确认后自动串行跑完四环节」口径，四环节（智能配置→规格稽核→资费校准→自动测试）逐环节推进、每环节止于反馈点，用户确认后才调度下一环节；② 全流程所有工作流处理完统一输出「下一步建议」引导；③ 3.2 提示词【限制】由自动串行纪律改为逐环节确认推进纪律；④ 1.1/1.2/1.3、2 章、3.2、6.1、6.2、6.5.3、7/8/10 章、附录 Checklist 同步；⑤ 业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约不变（工作流 JSON 与后端零改动，详见顶部 V3.6 履历） |
| V3.5（面板 req_id 透传 + 页面按 stage 动态联动） | 2026-09-20 | **结束节点面板 url 透传 `req_id` + 配置工作台页面按 `stage` 动态联动**：面板 url 追加 `&req_id=<实值>`（`gen_panel_code` 读取 `req_id`，00~06 绑定起始节点入参、07~10 为空串），url 目标形态 `config-workbench.html?offer_id=..&name=..&chatId=..&req_id=..&stage=<N>&view=stage`；`config-workbench.html` 解析 `req_id`，导航步进进度（已完成勾选 / 当前高亮）与默认 Tab 按 `stage` 动态加载（8 环节口径，08/09/10 辅助子流按就近展示）。生成器 + 13 个 JSON 同步（详见《工作流JSON开发规范.md》v1.6；顶部 V3.5 履历） |
| V3.4（页面单面板收敛） | 2026-09-19 | **结束节点页面地址收敛为单面板单 url（对齐参考示例）+ JSON 紧凑无空格**：各子工作流结束节点末尾 `{panel}` 由「配置工作台(view=workbench) + 环节业务页(view=stage)」双面板收敛为单面板——url 统一用环节业务页 `config-workbench.html?offer_id=..&name=..&chatId=<实值>&stage=<N>&view=stage`，`title` 用环节业务名，移除 `product-detail.html` 单品运营看板与 view=workbench 冗余面板；panel 输出 **JSON 紧凑无空格**（冒号/逗号后无空格，逐字对齐 `{"version":"1.0","message_id":"...","panels":[{"panel":"right","mode":"external","url":"...","title":"..."}]}`）；00/01 阶段 `offer_id` 取 `req_id` 兜底。生成器 + 11 个 JSON 同步，详见《工作流JSON开发规范.md》§十一（详见顶部 V3.4 履历） |
| V3.3（需求审批独立子流 + 方案确认门禁 + 8 环节口径） | 2026-09-19 | **工作流调用顺序对照历史 skill 版本优化**：① 需求工单审批须用户明确确认后才发起——`wf_sub_00` 提报单就绪即止于确认点，新增 `wf_sub_11_发起需求审批` 承接（自查 requirement_report→提取原文→submit_release_approval(approval-type=requirement)→回执）；② **需求分析生成《加载方案》后须用户明确确认才可继续智能配置**——`wf_sub_01` 输出方案即止于确认点，未回复确认前严禁调度 `wf_sub_02`（文档/提示词强化，JSON 不改）；③ 子流集 11→12 个（工作流 JSON 12→13 个）；④ 全流程统一 8 环节口径（受理验证并入环节6，不独立成节），全文标题头统一 `【环节N/8·全名】`，新增 8 环节总表；⑤ 1.1/1.2/1.3/1.4/2 章/3.2/3.3/6 章/7~10 章/附录 Checklist 同步；业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约不变（详见顶部 V3.3 履历） |
| V3.2（自动串行） | 2026-09-19 | **执行主干环节调用逻辑对照历史 skill 版本（flow-B）优化——回归"确认后自动串行"**：智能体提示词执行主干纪律重写——用户确认配置后**一次串行跑完四环节（`wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`），中途不停顿**，各环节成功后**立即打印该环节结果**并自动进入下一环节，**仅环节失败/异常时中断**；意图映射表"确认配置"行恢复为"一次串行跑完四环节"（删除"继续下一步"行）、限制条款改写为串行纪律、打印模板补"自动推进无需回复"说明；1.1/1.2/1.3/2 章/6.1（时序图）/6.3/6.5.3/7/8/9/10 章与附录 Checklist 同步；工作流 JSON 与后端零改动，业务口径/字段体系/子流编号/判定字段/fail_node 映射/req_id/落库 node_name/审批硬校验契约不变（详见顶部 V3.2 履历） |
...
**需求提报（【环节1/8】）→ 需求分析（【环节2/8】，五列模块表格《加载方案》）→ 用户确认 → 【智能配置（【环节3/8】配置落地）→ 规格稽核（【环节4/8】）→ 资费校准（【环节5/8】）→ 自动测试（【环节6/8】，含受理验证子集）】逐环节步进推进（**每环节处理完输出结果+下一步建议，用户反馈确认后才进入下一环节**）→ 上线审批（【环节7/8】上线校验看板，用户二次确认后发起）→ 监控运维（【环节8/8】确认上线后生成监控运维方案）** 的端到端闭环。

> 环节口径（V3.3）：全流程按 **8 环节** 编号——1 需求提报 / 2 需求分析 / 3 智能配置 / 4 规格稽核 / 5 资费校准 / 6 自动测试（**受理验证并入环节6，不独立成节**）/ 7 上线审批 / 8 监控运维。
...
> V3.2 关键调整：**执行主干环节调用逻辑对照历史 skill 版本（flow-B）优化——回归"确认后自动串行"**——用户确认配置后**一次串行跑完四环节（智能配置→配置规格稽核→资费校准→自动测试），中途不停顿**，各环节成功后立即打印该环节结果并自动进入下一环节（无需用户回复），**仅环节失败/异常时中断**，打印异常节点+原因并引导【重新执行】/【修改执行方案】。
> V3.6 关键调整（**替代 V3.2 自动串行口径**）：**执行主干改为人机协同步进式**——用户确认配置后**逐环节调度**（智能配置→规格稽核→资费校准→自动测试），**每一环节处理成功后先输出该环节结果 + 下一步建议，随即暂停等待用户反馈动作**，用户回复确认/继续（或指定下一步）后才调度下一环节；**仅环节失败/异常时提前中断**，打印异常节点+原因并引导【重新执行】/【修改执行方案】。**全流程所有工作流处理完后统一输出「下一步建议」引导**。
> V3.3 关键调整：① **需求工单审批须用户明确确认后才发起**——`wf_sub_00` 提报单就绪即**止于确认点**（不再直接发起审批），用户回复【发起需求审批】后由**新增子流 `wf_sub_11`** 承接；② **需求分析生成《加载方案》后须用户明确确认才可继续智能配置**——`wf_sub_01` 输出方案即**止于确认点**，未回复确认前严禁调度 `wf_sub_02`、严禁进入执行主干；③ 全文统一 **8 环节口径**（受理验证并入环节6，不独立成节），标题头统一 `【环节N/8·全名】`；④ 子流集 11→12 个（`wf_sub_00`~`wf_sub_11`）。

**全流程 8 环节总表（V3.3 口径，全文环节编号以此为准）：**

| 环节 | 全名 | 承载子流 | 关键动作 | 判定字段 |
| --- | --- | --- | --- | --- |
| 1 | 需求提报 | `wf_sub_00`（止于确认点）+ `wf_sub_11`（发起需求审批） | 要素提取 → CODE_RENDER_REQ 渲染提报单 → **提报单就绪即止于确认点**；用户回复【发起需求审批】后由 `wf_sub_11` 发起 `submit_release_approval`(approval-type=requirement) | 提报单就绪 / 审批单号 |
| 2 | 需求分析 | `wf_sub_01` | 模板轨：要素提取 → validate_elements 质量门禁 → merge_nested → validate_nested 本体校验闸 → render_table → 保存 requirement | plan_json / plan_md |
| 3 | 智能配置 | `wf_sub_02` | 自查 requirement → save_product_config → CODE_FUSION_GROUP_ECHO 融合成员回显 | status==SUCCESS/PARTIAL |
| 4 | 规格稽核 | `wf_sub_03` | 自查 config → realtime_spec_audit（组维度） | pass==1 |
| 5 | 资费校准 | `wf_sub_05` | 自查 config → check_billing_rule（check_scene=all，member_role 分组） | pass==1 |
| 6 | 自动测试（含受理验证） | `wf_sub_04` | 自查 config → offer_test → get_test_scenes → CODE_POLL_PROGRESS 轮询 → get_test_result → CODE_MAP_FIXED_CASES（31 条固定用例）→ 九章节报告 → 报告下载；**受理验证为其子集**（orderId/offerInstId 结论随测试报告输出，不独立成节） | test_passed==通过 |
| 7 | 上线审批 | `wf_sub_06` | CODE_SUMMARY_APPROVAL 汇总 → submit_release_approval(approval-type=launch) → 审批通过**自动上线 + 生成监控运维方案** | approval_id / status |
...
| 平台能力 | 在本场景中的角色 |
| --- | --- |
| 智能体（助手） | 产销品数字员工统一入口（对话式 + 流程式调度中枢）；由智能体（LLM）按 3.2 提示词【意图→工作流映射表】**语义识别直调** 12 个子工作流（**逐环节步进推进、每环节处理完输出结果+下一步建议、等待用户反馈后才调度下一环节**，异常引导由提示词【限制】约束），后端硬校验兜底；**需求工单审批须用户明确确认后才发起**（提报单就绪止于确认点，回复【发起需求审批】后由 `wf_sub_11` 发起）；流程结束后承接**消息查询**（审批进度/监控结果）与**异常处置引导**（重新执行/修改执行方案） |
| 工作流 | 承载 8 个业务环节的子工作流（`wf_sub_00`~`wf_sub_11`，共 12 个；req_id 单必填入参自查链路，无意图调度主流程）；由智能体语义识别后直调各子流，子流内部含环节结果存储节点（save_node_result） |
...
### 1.3 设计原则
1. **子工作流化 + 智能体语义调度 + 确定性下沉**：每个环节封装为子工作流（`wf_sub_00`~`wf_sub_11`，共 12 个；req_id 单必填入参自查链路）；无独立意图调度主流程，由智能体（LLM）按 3.2 提示词【意图→工作流映射表】语义识别后**直调**各子流。**智能体只做语义识别、业务理解与结果表达**；确定性纪律（req_id 生成与透传、环节落库、成败判定、fail_node 续跑、审批四环节硬校验）**不由提示词承载**，而由子流自查链路（query_node_result）+ type=6 代码节点 + 后端硬校验兜底（见 3.2 职责分层口径、6.1/6.2、4.4 端点契约）。
...
4. **确认后再执行**：需求分析生成《加载方案》后**须止于确认点并等待用户确认**，智能配置属于生产写入类操作，必须等用户对执行方案回复确认后才触发；配置数据直接取自执行方案 JSON，大模型不做二次加工，避免语义漂移。
...
8. **步进式推进，每环节输出下一步建议**：用户确认后的执行主干（智能配置→实时稽核→资费校准→自动测试）按固定顺序**逐环节推进、中途暂停等待用户反馈**，每完成一个环节即输出该环节处理结果 + **下一步建议**，经用户确认后才进入下一环节；**仅环节失败/异常时提前中断**，打印异常节点+异常原因，并引导用户【重新执行】或【修改执行方案】；全部成功后打印各环节成功详情并提示"是否发起上线审批"。
9. **审批二次确认（双轨）**：**需求工单审批**不自动触发，须在需求提报单生成后由用户明确回复【发起需求审批】才发起（`wf_sub_00` 提报单就绪即止于确认点，`wf_sub_11` 承接发起）；**上线审批**不自动触发，须在执行主干全部成功后由用户确认发起。
10. **人机协同（步进式）**：用户确认执行方案、逐环节确认下一步动作、确认发起需求工单审批、确认发起上线审批等决策点保留人工确认；**执行主干逐环节推进，每环节处理完输出结果+下一步建议并止于反馈点，用户确认后才进入下一环节**；流程结束后支持消息查询审批进度与监控运维结果。

### 1.4 实现方式说明（V2.0：Skills 技能包实现 → 13 工作流 JSON + 知识库 + 后端适配端点）

> 本方案第 2~6 章按原「九思平台（子工作流 + 插件 + 知识库切片）」实现方式编写，作为**业务口径与契约的权威定义**保留。**当前代码实现已重塑为 13 个工作流 JSON + `knowledge/` 知识库 + 后端 `/api/v1/appstore/*` 适配端点方式**，两者业务逻辑等价，对应关系如下：
...
| 子工作流 wf_sub_01~08（6.2 节节点表） | 13 个工作流 JSON / 12 个子工作流：`wf_sub_00`~`wf_sub_11`（环节覆盖扩展至需求提报工单审批独立子流/存量查询/存量合规/根因闭环等），`gen_workflows_v2.py` 确定性生成（无 `wf_main_intent` 意图调度主流程） |
...
| 业务环节 | 业务目标 | 平台实现载体 |
| --- | --- | --- |
| 需求提报（【环节1/8】） | 收集业务需求文档/结构化需求，生成需求工单；**需求工单审批须用户明确确认后才发起** | 子工作流 `wf_sub_00`（需求提报）**止于确认点**：LLM 要素提取（snake_case 扁平 JSON）+ CODE_RENDER_REQ 渲染提报单 + "是否发起【需求工单审批】？"，生成需求工单号 req_id；用户回复【发起需求审批】后由**新增子流 `wf_sub_11`（发起需求审批）** 承接：自查 requirement_report → 提取原文 → `submit_release_approval`（**approval-type=requirement**）→ 审批通过后流转 `wf_sub_01` |
| 需求分析（【环节2/8】） | 业务要素→配置字段映射，生成《产销品加载执行方案》并存储执行 JSON；**生成后止于确认点，须用户明确确认才可继续智能配置** | 子工作流 `wf_sub_01`（模板轨）：要素提取（CODE_EXTRACT_RECORD）→ CODE_VALIDATE_ELEMENTS 质量门禁 → CODE_MERGE_NESTED 合并嵌套 → CODE_OP_VALIDATE_NESTED 本体校验闸（validate-nested 端点）→ CODE_RENDER_TABLE 渲染表格 → 保存 requirement（node_name=requirement）→ **输出方案即止于确认点** |
| **用户确认** | 人工确认执行方案，确认后才进入配置落地 | 智能体提示词【限制】识别确认语义（方案就绪止于确认点，未命中确认不触发智能配置、不调度 `wf_sub_02`） |
| **执行主干**（步进式） | 智能配置→稽核→资费→测试按序推进，**每环节处理完输出结果+下一步建议、用户反馈后才进入下一环节**；异常打印节点与原因并引导重新执行/修改执行方案 | 智能体语义识别"确认配置"后**逐环节调度四环节** `wf_sub_02`→`wf_sub_03`→`wf_sub_05`→`wf_sub_04`（每环节止于反馈点，用户确认后再调度下一环节；失败即中断）；异常按 fail_node 续跑（门禁由后端硬校验） |
| 销售品智能配置（【环节3/8】） | 按 req_id 自查存储的执行方案 JSON，调用配置落地接口完成 CRM 配置 | 子工作流 `wf_sub_02`（req_id 单入参）：自查 requirement + 配置落地 `save_product_config` + **CODE_FUSION_GROUP_ECHO 融合组成员回显**（含可选 group） |
| 规格稽核（【环节4/8】） | 配置落地后**实时调用稽核接口**，同步取回稽核结果 | 子工作流 `wf_sub_03`（组维度）：实时稽核 `realtime_spec_audit`（组维度稽核，error_list 含 group 与 role 定位） |
| 资费校准（【环节5/8】） | 校验计费逻辑、优惠叠加冲突 | 子工作流 `wf_sub_05`（成员分组）：计费规则校验 `check_billing_rule`（check_scene=all，按 member_role 分组比对，E27 负值比对） |
| 销售品自动测试（【环节6/8】，含受理验证） | **发起**销售品自动化测试（发起动作）→轮询进度→取结果；测试平台自动执行受理类场景即完成受理验证（**受理验证为环节6子集，不独立成节**） | 子工作流 `wf_sub_04`（重写）：`offer_test` / `get_test_scenes` / CODE_POLL_PROGRESS 轮询 / `get_test_result` + **CODE_MAP_FIXED_CASES（31 条固定用例 ACC/BILL/CUST + P0/P1）** + 九章节正式版报告 + 报告下载（report/download） |
| 上线审批（【环节7/8】） | 汇总测试报告（含受理验证）、推送审批；**审批通过自动上线 + 生成监控运维方案** | 子工作流 `wf_sub_06`（**双轨 requirement/launch**）：CODE_SUMMARY_APPROVAL 汇总报告 → `submit_release_approval` 推送审批，审批通过自动上线并生成监控运维方案 |
| **审批进度查询**（辅助） | 用户发送消息查询审批单当前状态（**需求工单审批/上线审批双轨**） | 子工作流 `wf_sub_08`（审批进度查询，**双轨**查询）+ `query_approval_status`（按 approval_id/offer_id） |
| **存量产品查询**（辅助） | 用户发送消息查询存量/在售产品销售品信息（只读） | 子工作流 `wf_sub_09`（存量产品查询，**只读**）+ CODE_OP_QUERY_OFFER + `query_similar_offer`/`query_node_result` 只读查询 |
| **存量合规扫描**（辅助） | 存量销售品合规扫描（上架合规） | 子工作流 `wf_sub_10`（存量合规扫描）：CODE_OP_SHELF_COMPLIANCE → 后端 `/shelf-compliance` 端点 |
| 监控运维（【环节8/8】） | 上线后持续监控异常，异常根因推理 + 建工单闭环 | 子工作流 `wf_sub_07`（异常分支）：`query_product_monitor` 监控查询 → 异常时 **CODE_OP_ROOT_CAUSE 根因推理**（/ops/root-cause）+ **CODE_OP_CREATE_WO 建工单**（/ops/work-orders）闭环 |
...
| 助手名称 | 产销品数字员工 |
| 功能介绍 | 面向产销品域的数字员工，覆盖需求提报→需求分析（五列模块表格《加载方案》）→用户确认→智能配置→规格稽核→资费校准→自动测试（含受理验证）→上线审批→监控运维的 **8 环节**全流程自动化。 |
| 发布范围 | 所有人可见 |
| 图标 | 默认/上传产销品图标 |

### 3.2 提示词（角色 + 技能 + 限制 模式）

> 本节提示词为**智能体语义识别 + 子工作流直调**的运行时定义，按【角色 + 技能 + 意图映射表 + 限制】组织。**职责分层口径（本版本重构确立）**：智能体（LLM）只负责三件事——① **语义识别**：把用户消息按下方【意图→工作流映射表】映射到一个业务动作并直调对应子工作流；② **业务理解/补全**：在需求分析环节理解需求、提取要素、补全字段、生成《加载方案》（五列模块表格）；③ **结果表达**：按固定模板输出环节结果、下一步建议、异常引导、汇总块。**以下技术实现细节不属于提示词职责，由确定性载体保障、提示词不描述、不要求 LLM 管理**：req_id 的生成与透传、环节结果落库（save_node_result / node_name）、环节成败的判定（status/pass/test_passed 等出参编码）、fail_node 续跑与存储回放、审批四环节硬校验——这些由子工作流内部 type=6 代码节点 + 自查链路（query_node_result）+ 后端适配端点硬校验兜底（见 6.2 子流表、4.4 端点契约、1.4 实现方式）；LLM 仅按子工作流返回的结果做展示与引导。**全流程按 8 环节口径**（受理验证并入环节6，不独立成节），标题头统一 `【环节N/8·全名】`。**全流程为步进式人机协同（V3.6 口径）**：每个子工作流处理成功后，智能体先输出该环节结果 + 下一步建议，随即**暂停等待用户反馈动作**，用户确认/继续（或指定下一步）后才直调下一环节子工作流；仅环节失败时提前中断。

```
【角色】
你是安徽电信产销品域数字员工，精通 CPCP 产销品管理、CRM 配置、计费规则、订单受理
与测试验证，负责销售品从需求到上线的端到端自动化加载（业务理解与表达）。你不直接操作
CRM、不修改配置、不代替用户做最终业务决策；所有落地动作由对应子工作流在内部完成，
你只负责识别意图、直调子工作流、并按返回结果向用户展示与引导。

【技能】
1. 需求理解与《加载方案》生成（【环节1/8】+【环节2/8】）：理解需求原文→按
   3 模块/9 分类/24 字段拆解要素→补全→输出《加载方案》（五列模块表格 + 执行方案）。
   补全原则：需求原文最高优先、不得虚构；字段来源仅"原始需求/AI补全"两态；仅套餐档位
   （价格）缺失且无参照才标"待补充"（禁推理价格）；套餐编码标"系统待生成"；有待补充
   则列出并提示用户补充。**需求提报单生成后必须提示"是否发起【需求工单审批】"并止于
   确认点**，用户明确回复【发起需求审批】后才发起，未表态则停留等待、禁止擅自发起。
   **执行方案生成后同样止于确认点**并提示"请确认执行方案，回复【确认配置】后开始配置
   落地"，用户明确回复确认类语句（确认配置/确认执行/同意/可以/执行吧）前不得进入执行主干。
2. 执行主干引导（【环节3/8】~【环节6/8】）：用户确认执行方案后，按子工作流顺序**逐环节
   推进**——每环节子工作流返回结果后，立即输出该环节结果 + 下一步建议，并**暂停等待用户
   反馈**，用户确认/继续（继续/下一步/同意等）后才直调下一环节子工作流；仅环节失败/异常
   时中断。环节成败以子工作流返回结果为准（由子流内部判定），你不得自行推断。
3. 审批与上线（【环节7/8】）：执行主干四环节全部成功后，输出【执行主干全部完成】汇总块
   （每环节一行：环节/执行结果/关键数据）并提示"是否发起上线审批"；用户确认后才发起。
   审批结果、上线动作、监控运维方案生成均由子工作流完成，你仅展示结果与下一步建议。
4. 查询与运维（【环节8/8】+辅助）：按用户消息直调 审批进度查询 / 监控运维 / 存量产品
   查询 / 存量合规扫描 对应子工作流并展示结果。

【意图→工作流映射表（语义识别 → 业务动作 → 直调子工作流）】
| 用户意图 | 语义识别后的业务动作 / 直调子工作流 | 业务说明 |
| --- | --- | --- |
| 提报/修改需求（【环节1/8】） | 需求提报 → 先生成提报单，**止于确认点**；用户回复【发起需求审批】后再发起需求工单审批；审批后进入需求分析 | requirement_text；req_id 由子流生成/透传，LLM 不管理 |
| 发起需求工单审批（【环节1/8】） | 发起需求工单审批（须用户明确确认后才发起） | 发起即止于回执展示 |
| 生成加载方案（【环节2/8】） | 需求分析，输出《加载方案》（五列表格+执行方案），**止于确认点**：用户未确认前不得进入执行主干 | |
| 确认配置（【环节3/8】~【环节6/8】） | **执行主干逐环节步进推进**：智能配置 → 规格稽核 → 资费校准 → 自动测试（含受理验证）；每环节处理完输出结果+下一步建议并列于反馈点，用户确认后才推进下一环节，失败即中断 | |
| 重新执行失败环节 | 从上次失败环节继续执行（子流自查自动续跑，已成功环节不重复） | |
| 上线审批（【环节7/8】） | 发起上线审批（用户确认后发起；子流内部校验四环节结果齐全） | |
| 查询审批进度（辅助） | 审批进度查询（需求工单审批/上线审批） | approval_id/offer_id |
| 查询监控运维（【环节8/8】） | 监控运维查询（异常根因+工单闭环） | offer_id |
| 查询存量产品（辅助） | 存量产品查询（只读） | 产品名/ID |
| 存量合规扫描（辅助） | 存量合规扫描 | 存量范围 |
| 业务问答/超范围 | 按 K1~K5 知识库检索回答；超范围按答案为空提示 | 检索词 |

【限制】
1. 仅回答产销品加载相关业务，超范围按"答案为空"提示。
2. **执行方案确认门禁**：需求分析生成《加载方案》后**必须止于确认点**——输出五列模块
   表格 + 执行方案 JSON 并提示"请确认执行方案，回复【确认配置】后开始配置落地"，随即
   **停止等待用户回复**；用户明确回复确认前，不得进入执行主干、不得发起配置落地。
3. 确认语义未命中绝不进入执行主干；执行主干不跳步、不并行。
4. **需求工单审批须用户明确确认后才发起**：需求提报单生成后必须提示"是否发起【需求
   工单审批】"，用户未明确回复【发起需求审批】前不得发起审批。
5. **逐环节确认推进**：执行主干按序推进（智能配置→规格稽核→资费校准→自动测试），
   每环节结果输出 + 下一步建议后**暂停等待用户反馈**，用户确认（继续/下一步/同意等）后
   才推进下一环节，严禁未经确认擅自推进；仅环节失败/异常时提前中断。
6. **结果表达铁律**：环节结果、✅ 与统计值必须与子工作流返回结果一一对应，禁止凑数、
   虚构、自行推断成败；不泄露资费配置明细；执行主干全部成功后输出【执行主干全部完成】
   汇总块并按模板提示下一步动作。
7. **异常处置**：环节失败/异常时中断，按异常模板输出环节/原因/明细/建议，引导用户回复
   【重新执行】（从失败环节续跑）或【修改执行方案】；不得自行重试，也不得输出汇总表格。
8. **职责不越界**：你只做语义识别、业务理解与结果表达；req_id 透传、环节落库、成败判定、
   续跑回放、审批硬校验等由子工作流/代码节点/后端保障，你不得自行构造、修改或绕过。
```
...
| 工作流 | 挂载 **12 个子工作流 JSON**（`wf_sub_00`~`wf_sub_11`，见 `工作流配置/智能体工作流集V1.6/`，由 `gen_workflows_v2.py` 生成；无意图调度主流程，由智能体按 3.2 节【意图→工作流映射表】语义识别后**直调**；其中 `wf_sub_11` 为需求工单审批独立子流） |
...
> **核心口径**：无独立意图调度主流程，调度职责回归智能体（LLM）——按 3.2 节提示词【意图→工作流映射表】语义识别后**直调** 12 个子工作流 `wf_sub_00`~`wf_sub_11`。常驻提示词精简（角色/技能/映射表/限制），**全流程为步进式人机协同**（每环节处理完输出结果+下一步建议、用户反馈后才调度下一环节）；结果打印与异常引导由【限制】约束，确认门禁（方案确认/需求审批确认/逐环节确认）亦由【限制】约束。**确定性纪律不承载于提示词**——req_id 生成与透传、环节成败判定、fail_node 续跑、存储回放、审批四环节硬校验等由子工作流自查链路（query_node_result）+ type=6 代码节点 + 后端硬校验（四环节门禁/req_id 校验）兜底（见 3.2 职责分层口径）。子工作流内部保留环节结果存储节点（save_node_result），续跑按存储记录回放。**全流程按 8 环节口径**（受理验证并入环节6，不独立成节），标题头统一 `【环节N/8·全名】`。
>
> **选择智能体语义直调的理由**：① 平台配置规范要求"调度中枢用智能体"（见《SitechAI开发平台配置规范.md》），独立调度主流程与规范职责重叠冗余；② 独立调度主流程存在真实缺陷（悬挂引用、未路由主业务链路），无法触达全链路；③ 「确认执行」后携带 req_id 的注入问题由子流自查链路（query_node_result 按 req_id 自查）在子流内部解决，无需主调度注入；④ 串行/防跳步/续跑等确定性纪律由子流自查链路 + 代码节点 + 后端硬校验保障（不依赖 LLM 自觉），行为仍可审计、可测试。

**执行主干调度时序（智能体语义识别"确认配置"后逐环节步进推进，每环节止于反馈点）：**

```
前置：wf_sub_01 已生成《加载方案》并**止于确认点**（未回复确认前智能体不调度 wf_sub_02）
...
② 确认语义由智能体提示词【限制】识别（**无需写 CONFIRMED 确认标记**——后端
    save_product_config 已移除该门禁，识别命中即调度）
  ▼
③ 【环节3/8·智能配置】直调 wf_sub_02 智能配置（入参 req_id；子流自查 requirement→提取执行方案原文→落地）
    出参 status/offer_id/save_result
    ├─ status==SUCCESS/PARTIAL → 打印【环节3/8·智能配置结果】+ 下一步建议 → 子流内部环节结果存储已写入
    │   （wf_sub_02 内置 save_node_result，node_name=config，req_id=入参）
    └─ 异常 → 中断，打印异常节点+原因
    → 成功即**止于反馈点，等待用户确认后才进入环节4（回复「继续/下一步」推进）**
  ▼
④ 【环节4/8·规格稽核】直调 wf_sub_03 规格稽核（入参 req_id，子流自查 config 取 offer_id/config_json）
    出参 pass/error_list/audit_summary
    ├─ pass==1 → 打印【环节4/8·规格稽核结果】+ 下一步建议 → 子流内部已存 node_name=spec
    └─ pass==0/接口异常 → 中断，打印异常
    → 成功即**止于反馈点，等待用户确认后才进入环节5（回复「继续/下一步」推进）**
  ▼
⑤ 【环节5/8·资费校准】直调 wf_sub_05 资费校准（入参 req_id，子流自查 config 取 config_json）
    出参 pass/risk_list
    ├─ pass==1 → 打印【环节5/8·资费校准结果】+ 下一步建议 → 子流内部已存 node_name=fee
    └─ 不通过 → 中断，打印异常
    → 成功即**止于反馈点，等待用户确认后才进入环节6（回复「继续/下一步」推进）**
  ▼
⑥ 【环节6/8·自动测试（含受理验证）】直调 wf_sub_04 自动测试（入参 req_id，子流自查 config 取 offer_id）
    出参 globalId/测试报告（含受理验证结论 orderId/offerInstId）
    ├─ test_passed==通过 → 打印【环节6/8·自动测试结果】+ 下一步建议 → 子流内部已存 node_name=test
    └─ 失败/超时 → 中断，打印异常
  ▼
    ⑦ 智能体汇总打印各环节成功详情 + 提示"是否发起上线审批"
    ………… 用户回复"发起审批" → ⑧ 【环节7/8·上线审批】直调 wf_sub_06（入参 req_id：
    子流串行自查 5 类环节结果 → 子流生成报告 → 报告存储(node_name=report)
    → submit_release_approval 推送审批）→ 输出审批单号 → 审批通过自动上线 +
    生成监控运维方案（【环节8/8·监控运维】）
```

**调度要点（确定性契约的权威描述，与 3.2 提示词职责分层互补：技术确定性由本处保障，提示词只做语义识别与确认门禁）：**
1. **确认门禁**：**需求分析生成《加载方案》后须止于确认点**——`wf_sub_01` 输出方案即停止等待，未命中确认语义前智能体不调度 `wf_sub_02`。后端 `save_product_config` 不校验 confirmed 入参与 CONFIRMED 标记——确认与否由智能体提示词【限制】语义识别判断（提示词职责）；`submit_release_approval` 仍校验 req_id 四环节结果齐全（执行主干未走完不得发起审批）。
2. **逐环节步进推进纪律**：确认后按序逐环节推进（02→03→05→04），严禁并行调用、跳过环节；环节成败由子流/代码节点按工具出参字段（status/pass/test_passed）裁决，LLM 不凭语义推断成败；**每环节必须得到用户反馈后才调度下一环节，严禁未经确认擅自推进**；**仅环节失败/异常时中断**。
...
**每环节结果打印格式（LLM 输出统一模板，N 取值 3~6 及全流程各环节通用）：**
```
【环节N/8·{环节名称}】✅ 执行成功
- 关键数据：{该环节关键输出（如 offer_id/save_result、audit_summary、
  risk_list 为空、测试统计+受理验证小节 orderId/offerInstId）}
- 下一步建议：确认后可继续进入【下一环节名称】——请回复【下一步】/【继续】
  推进（如需修改可回复【修改执行方案】；末环节改为输入"发起审批"提交上线审批）
```
> 每环节成功输出结果 + **下一步建议** 后**止于反馈点、暂停等待用户回复**，用户确认（下一步/继续/同意）后才进入下一环节；**仅环节失败/异常时中断**，等待用户反馈后再行动。末环节成功后提示"是否发起上线审批"。
...
```
> 环节异常即**中断**（步进推进中止），等待用户反馈后再行动。

**成功结果详情输出格式（四环节步进推进全部成功后统一模板）：**
```
【执行主干全部完成】✅ 共4个环节执行成功：
1. 【环节3/8·智能配置】：offer_id={...}，四类字段全部写入成功；
2. 【环节4/8·规格稽核】：通过，{audit_summary}；
3. 【环节5/8·资费校准】：通过，未发现叠加/互斥冲突；
4. 【环节6/8·自动测试（含受理验证）】：场景 N 个、测点 M 个全部一致；
...
### 6.2 子工作流拆分（12 个子工作流 JSON）

> 实现口径（与 `工作流配置/智能体工作流集V1.6/` 导出 JSON 一致，`gen_workflows_v2.py` 确定性生成）：**12 个子工作流 `wf_sub_00`~`wf_sub_11`（无独立意图调度主流程，由智能体按 3.2 提示词【意图→工作流映射表】语义识别直调）**。各子工作流统一 **req_id 单必填入参**（唯一批次标识，方案批次与执行主干共用；由 wf_sub_00 需求提报代码节点生成），子流程内部通过 query_node_result 自查所需上游结果，并在结束前通过**内置环节结果存储节点**（save_node_result，req_id=入参，node_name=本环节名）落库；确定性逻辑内嵌为 **type=6 代码节点**（CODE_*）。全流程按 **8 环节**口径（受理验证并入环节6，不独立成节）。**各子工作流（工作流）处理成功后由智能体输出该环节结果 + 【下一步建议】引导，止于反馈点等待用户反馈后才调度下一环节（步进式人机协同，见 6.1 节）**。

| 子工作流 | 编码 | 入参 | 输出 | 关键节点 / 代码节点 |
| --- | --- | --- | --- | --- |
| 需求提报（【环节1/8】，止于确认点） | `wf_sub_00` | requirement_text, requirement_file | req_id / 需求工单号 / 提报单 | 开始 → LLM 要素提取（snake_case 扁平 JSON）→ **CODE_RENDER_REQ 需求提报确认渲染**（render_requirement_report）→ **终止于确认点**（输出提报单 + "是否发起【需求工单审批】？"，**不直接发起审批**），node_name=requirement_report；用户回复【发起需求审批】后转 `wf_sub_11` |
| 发起需求审批（【环节1/8】） | `wf_sub_11` | req_id | approval_id / status | 开始(req_id) → 自查需求工单(query_node_result, node_name=requirement_report) → CODE_EXTRACT_RECORD(取原文) → **submit_release_approval（approval-type=requirement，用户确认后发起）** → LLM 回执渲染 → 结束 |
| 需求分析（【环节2/8】，止于确认点） | `wf_sub_01` | req_id | plan_json / plan_md（五列模块表格） | 开始 → CODE_EXTRACT_RECORD（取需求工单要素）→ 产品准入 LLM（选模板）→ query_similar_offer（相似产品 offerInfo）→ **CODE_GET_TEMPLATE 模板注册表选配**（/方案/templates/_registry.json）→ 要素提取 LLM → **CODE_VALIDATE_ELEMENTS 要素提取质量门禁**（validate_elements）→ **CODE_MERGE_NESTED 嵌套合并**（merge_nested）→ **CODE_OP_VALIDATE_NESTED 本体校验闸**（/validate-nested，R-C04/C06）→ **CODE_RENDER_TABLE 渲染四列表格**（render_table）→ 保存 requirement（node_name=requirement）→ **输出方案即止于确认点**（未确认前严禁调度 `wf_sub_02`）→ 结束 |
| 智能配置（【环节3/8】配置落地） | `wf_sub_02` | req_id（必填） | offer_id / save_result（含可选 group） | 开始(req_id) → query_node_result(自查 requirement) → CODE_EXTRACT_RECORD(取 result_json 原文) → save_product_config(配置落地) → **CODE_FUSION_GROUP_ECHO 融合组成员回显**（含可选 group/role/offer_id 成员集）→ **save_node_result(node_name=config)** → 输出结果+下一步建议 → 结束（止于反馈点，确认后转 `wf_sub_03`） |
| 规格稽核（【环节4/8】，组维度） | `wf_sub_03` | req_id（必填） | pass / error_list / audit_summary | 开始(req_id) → 自查 config（取 offer_id/config_json）→ CODE_EXTRACT_RECORD → realtime_spec_audit(实时稽核, **组维度稽核**, error_list 含 group:role 定位) → 大模型(整改建议生成) → **save_node_result(node_name=spec)** → 输出结果+下一步建议 → 结束（止于反馈点，确认后转 `wf_sub_05`） |
| 资费校准（【环节5/8】，成员分组） | `wf_sub_05` | req_id（必填） | pass / risk_list | 开始(req_id) → 自查 config（取 config_json）→ CODE_EXTRACT_RECORD → check_billing_rule(check_scene=all，**按 member_role 分组比对，E27 负值比对**) → 大模型(风险解读) → **save_node_result(node_name=fee)** → 输出结果+下一步建议 → 结束（止于反馈点，确认后转 `wf_sub_04`） |
| 自动测试（【环节6/8】，含受理验证，重写） | `wf_sub_04` | req_id（必填） | globalId / 测试报告（九章节正式版） | 开始(req_id) → 自查 config（取 offer_id）→ offer_test(发起) → get_test_scenes → **CODE_POLL_PROGRESS 轮询进度**（type=6，asyncio.sleep 5s×360/30 分钟）→ get_test_result → **CODE_MAP_FIXED_CASES 确定式 31 条固定用例映射**（ACC/BILL/CUST + P0/P1 + 缺省 + E26 核对）→ 九章节正式版测试报告 → **CODE_DOWNLOAD_TEST_REPORT 报告下载链接**（/report/download）→ **save_node_result(node_name=test)** → 输出结果+下一步建议 → 结束（止于汇总点，提示是否发起上线审批） |
| 上线审批（【环节7/8】，双轨） | `wf_sub_06` | req_id（必填） | approval_id / status | 开始(req_id) → 自查 四环节结果 → **CODE_SUMMARY_APPROVAL 审批汇总**（requirement/launch 双轨：requirement 轨=需求工单审批，launch 轨=上线审批，后端硬校验四环节）→ submit_release_approval(推送审批) → 审批通过后**自动上线 + 生成监控运维方案**（CODE_DOWNLOAD_LAUNCH_SCRIPT 上线脚本下载 /script/download，/*run@crm*/+/*run@billing*/）→ 结束 |
| 监控运维（【环节8/8】，异常分支） | `wf_sub_07` | offer_id / date_range | 指标与告警 / 工单号 | 开始 → query_product_monitor → 选择器(异常?) → 异常分支 **CODE_OP_ROOT_CAUSE 根因推理**（/ops/root-cause）→ **CODE_OP_CREATE_WO 建工单闭环**（/ops/work-orders）→ 结束；正常分支 send_alert/汇总 |
| 审批进度查询（辅助，双轨） | `wf_sub_08` | approval_id / offer_id | 审批状态摘要 | 开始 → query_approval_status（**双轨**：approval_id 或 offer_id）→ 大模型(状态摘要归纳) → 结束 |
| 存量产品查询（辅助，只读） | `wf_sub_09` | 产品名称 / 产品 ID | 存量销售品信息 | 开始 → **CODE_OP_QUERY_OFFER 只读查询** → query_similar_offer / query_node_result（只读，不落库不写）→ 结束 |
| 存量合规扫描（辅助） | `wf_sub_10` | 存量产品范围 | 合规扫描结论 | 开始 → **CODE_OP_SHELF_COMPLIANCE 合规扫描**（/shelf-compliance）→ 结束 |

> **实现状态说明**：上表 12 个子工作流 JSON 为当前实现基线（`工作流配置/智能体工作流集V1.6/`，`gen_workflows_v2.py` 生成；另有 2 份合并 export JSON 供平台联调参考）。以 12 子工作流 JSON + 智能体提示词语义调度 + knowledge/ 知识库 + 后端 `/api/v1/appstore/*` 适配端点为准。`wf_sub_11` 为 V3.3 新增（需求工单审批独立子流）。
>
...
- **步进式推进**：环节成功后输出结果+下一步建议并**止于反馈点、暂停等待用户回复**，用户确认（下一步/继续/同意）后才**进入下一环节**；**仅环节失败/异常时中断**（逐环节推进 02→03→05→04，每环节需用户反馈后才能推进下一环节）。
- **异常处置**：环节异常时中断，按"异常处置输出格式"模板输出异常环节名称、异常原因（引用接口返回原文，不臆测）、关键明细与整改建议，并引导【重新执行】/【修改执行方案】，不得自行重试。
...
**⑨ 审批发起确认（由智能体语义识别触发）**
- 四环节步进推进全部成功后，智能体输出"成功结果详情 + 是否发起上线审批"提示，等待用户回复。
...
> 本节为 6.1~6.3 节平台工作流描述的代码实现等价物（对应关系见 1.4 节）。当前实现基线与 `工作流配置/智能体工作流集V1.6/` 导出的 12 个子工作流 JSON 一致，由 `gen_workflows_v2.py` 确定性生成；业务逻辑与第 2~6 章平台工作流口径严格等价，仅是「实现载体」迁移——确定性逻辑化为工作流 type=6 代码节点 + 智能体提示词语义调度 + `knowledge/` 知识库 + 后端 `/api/v1/appstore/*` 适配端点。

#### 6.5.1 工作流 JSON 集结构（12 个子工作流）

```
工作流配置/智能体工作流集V1.6/            # 12 个子工作流 JSON 集（gen_workflows_v2.py 确定性生成；无意图调度主流程）
├── wf_sub_00_需求提报               # 要素提取→render_requirement_report→止于确认点（不直接发起审批）
├── wf_sub_11_发起需求审批           # 用户确认后：自查需求工单→submit_release_approval(approval-type=requirement)
├── wf_sub_01_需求分析               # 模板轨：要素提取→validate_elements→merge_nested→validate_nested→render_table→保存 requirement→输出方案止于确认点
...
> 说明：上表 12 个子工作流均为完整独立工作流（自带开始节点/req_id 入参/自查链路/结束节点），由智能体按 3.2 提示词【意图→工作流映射表】语义识别后直调。`wf_sub_11` 为 V3.3 新增。

#### 6.5.2 子工作流 ↔ 关键代码节点对照

| 子工作流 | 关键代码节点（type=6） |
| --- | --- |
| 需求提报 `wf_sub_00` | CODE_RENDER_REQ（render_requirement_report 需求提报单渲染）→ 止于确认点 |
| 发起需求审批 `wf_sub_11` | CODE_EXTRACT_RECORD（取需求工单原文）→ 需求工单审批 submit_release_approval（approval-type=requirement） |
...
意图路由规则与 3.2 节【意图→工作流调度映射表】等价：首次提报/修改需求→wf_sub_00（**止于确认点**）；发起需求工单审批→wf_sub_11（须用户明确确认）→审批通过后 wf_sub_01（**方案生成后止于确认点**）；确认配置→**逐环节步进推进四环节** wf_sub_02→wf_sub_03→wf_sub_05→wf_sub_04（每环节止于反馈点，用户确认后才推进下一环节，失败即中断）；重新执行失败环节→按 fail_node 映射续调（STAGE1_CONFIG→wf_sub_02、STAGE2_AUDIT→wf_sub_03、STAGE3_FEE→wf_sub_05、STAGE4_TEST→wf_sub_04，req_id 沿用原值）；发起审批→wf_sub_06；审批通过自动上线+监控运维方案→wf_sub_06 尾段；审批进度查询→wf_sub_08；监控运维→wf_sub_07（异常分支根因闭环）；存量查询→wf_sub_09；存量合规扫描→wf_sub_10；业务问答→按 K1~K5 知识库检索；超范围→按答案为空提示回复。

调度纪律（业务交互侧由 3.2 提示词【限制】约束语义识别与确认门禁，技术确定性侧由子流自查链路 + 代码节点 + 后端硬校验兜底，**不依赖 LLM 自觉**）：**执行方案未确认不得进入执行主干**（wf_sub_01 方案就绪止于确认点，未回复确认不调度 wf_sub_02）；确认语义未命中不得进入执行主干；**需求工单审批须用户明确确认后才发起**（wf_sub_00 提报单就绪止于确认点，未回复【发起需求审批】前严禁调度 wf_sub_11）；执行主干必须按序**逐环节步进推进**（02→03→05→04，**每环节处理完输出结果+下一步建议并止于反馈点，用户确认后才调度下一环节，严禁未经确认擅自推进下一环节**）、严禁跳过环节、严禁并行调用、严禁重复调用已成功环节（续跑时已成功环节按存储记录回放）；环节成败由子流/代码节点依据后端出参裁决（status/pass/test_passed），失败即中断引导（LLM 不自行推断成败）；审批后端四环节硬校验兜底。
...
| 配置规范条款 | 本方案落点（对齐自证） |
| --- | --- |
| **平台四层架构**（智能体/插件/工作流/知识库） | 本方案即为四层落地：智能体（3 章产销品数字员工，含语义调度）、插件（4 章自研 8 工具 + V2.0 后端适配端点）、工作流（6.5 章 12 个子工作流 JSON）、知识库（5 章 K1~K5 + `knowledge/` 结构化资产）。 |
| **源码驱动纪律**（JSON 是生成器产物、禁止手改、改生成器后重新生成 + 全量校验 ALL_OK） | 12 个子工作流 JSON 全部由 `gen_workflows_v2.py` 确定性生成（6.5.1），插件由 `gen_plugins.py` 生成；改业务逻辑一律改生成器后重新生成并做 JSON 契约校验（导入平台自动校验 ALL_OK 项，见交付运维手册 2.3）。**禁止直接手改 `智能体工作流集V1.6/*.json`**。 |
| **节点 type 全集**（0开始/1LLM/2条件分支/3HTTP插件/6代码节点/9结束/13子流程） | 全部 12 个子工作流仅使用上述 type 值；无 type=5 以及任何未枚举类型；无**循环节点**（type 全集之外的循环类型不出现），轮询统一用 type=6 代码节点 CODE_POLL_PROGRESS（asyncio.sleep 5s×360/30 分钟，见 6.5.4）。 |
...
| **nid UUID 形态**（`a1b2c3d4-0000-4000-8000-{12位seq}`，seq 按子流分段） | 生成器按规范生成 nid：各子流 `wf_sub_00`~`wf_sub_11` 的 seq 分段递增，满足"单次生成全库唯一、seq 按子流分段"约束。 |
...
| 阶段4 工作流编排 | 由 `gen_workflows_v2.py` 生成 **12 个子工作流 JSON**（`wf_sub_00`~`wf_sub_11`，无 `wf_main_intent` 意图调度主流程，见 `工作流配置/智能体工作流集V1.6/`）并导入调试；各子流 type=6 代码节点（CODE_EXTRACT_RECORD/CODE_VALIDATE_ELEMENTS/CODE_MERGE_NESTED/CODE_RENDER_TABLE/CODE_MAP_FIXED_CASES/CODE_POLL_PROGRESS 等）、req_id 单必填自查链路、环节结果存储节点联调 | 已生成并导入的 12 个子工作流 JSON | 6天 |
| 阶段5 智能体集成 | 提示词、插件、工作流、知识库装配；模型参数调优 | 已发布智能体 | 2天 |
| 阶段6 验证与优化 | 以《产品信息.txt》销售品改写需求样例跑通全流程；验证**执行方案确认门禁**（《加载方案》生成后止于确认点，回复确认前不调度 wf_sub_02）、"未确认不配置"约束（智能体确认语义识别）、**需求工单审批须用户确认后发起**（提报单就绪止于确认点，回复【发起需求审批】后才调度 wf_sub_11）、智能体步进式推进（确认后逐环节推进、每环节处理完输出结果+下一步建议并止于反馈点、用户确认后才推进下一环节）与结果打印、异常中断引导与续跑、审批发起门禁（四环节硬校验）、消息查询（审批进度/监控结果）、测试报告受理验证结论完整性；提示词迭代 | 验证报告 | 4天 |
| 合计 | — | — | 约22个工作日 |
...
2. **工作流级**：
   - 正向用例：完整合规需求（取《产品信息.txt》中某销售品资费规则改写）→ **需求提报单生成并止于确认点 → 用户回复【发起需求审批】→ wf_sub_11 发起需求工单审批（approval-type=requirement）** → 执行方案生成（节点结果存储保存）→ **用户确认（智能体识别确认语义，无需写标记）** → 智能体步进式推进执行主干（确认后**逐环节调用** wf_sub_02→03→05→04，每环节打印结果+下一步建议后**止于反馈点，用户回复【下一步】/【继续】确认后才进入下一环节**）→ 全部成功后打印成功详情并提示"是否发起审批" → 用户确认 → 审批单生成；
   - 反向用例：**未确认执行方案直接要求配置 → 智能体按提示词拒绝调度智能配置（确认语义识别）**；**需求分析生成《加载方案》后用户未表态 → 智能体止于确认点、不调度 `wf_sub_02`（严禁擅自进入执行主干）**；**提报单生成后用户未表态 → 智能体止于确认点、不发起审批（严禁擅自调度 wf_sub_11）**；**主干全部成功后未确认直接要求审批 → 调度 wf_sub_06 后 submit_release_approval 因四环节结果校验拒绝/按提示词先引导确认**；构造资费冲突（如叠加优惠互斥）→ 资费校准拦截并中断引导；构造配置缺陷 → 实时稽核驳回（pass=0）中断引导；
   - 步进式推进用例：确认后 → 验证智能体**逐环节调用、每环节处理完输出结果+下一步建议并止于反馈点、不等待用户回复不推进下一环节**；只有用户回复【下一步】/【继续】后才进入下一环节（不跳环节、不重复调用）；
   - 异常中断用例：稽核不通过 → 验证智能体中断、打印异常节点"【环节4/8·规格稽核】"+原因+建议、引导重新执行/修改执行方案；选择【重新执行】→ 验证从稽核环节续调且不重复落地；选择【修改执行方案】→ 验证回到需求分析；
...
| 指标 | 目标 |
| --- | --- |
| 端到端流程贯通率 | 100%（给定需求→执行方案→确认→执行主干步进推进（配置落地→实时稽核→资费→测试报告含受理验证，每环节输出结果+下一步建议）→成功详情+审批确认→审批单生成） |
| 确认门禁有效性 | 未确认时配置落地触发率 = 0%（智能体确认语义识别）；未确认发起审批时审批推送触发率 = 0%（四环节结果校验）；**需求工单审批未确认时发起率 = 0%（提报单就绪止于确认点，未回复【发起需求审批】不调度 wf_sub_11）**；**执行方案未确认时智能配置触发率 = 0%（方案就绪止于确认点，未回复确认不调度 wf_sub_02）**；**执行主干每一环节未获用户反馈确认时，不得推进下一环节（未确认不推进率 = 0%，未确认不推进）** |
| 步进式推进纪律 | 确认后逐环节推进：每环节处理完 100% 输出该环节结果 + 下一步建议并止于反馈点，用户确认后才进入下一环节；无跳步、无并行调用、无未经确认擅自推进下一环节、无重复调用；每环节结果打印率 100% |
| 异常处置完整性 | 异常中断时 100% 输出异常节点名称、异常原因、整改建议，并给出重新执行/修改执行方案引导 |
...
| 剧本环节 | 对应验收内容 | 演示方式 |
| --- | --- | --- |
| 幕1 需求提报/分析 | 需求要素映射准确率 + 执行方案生成 | 输入需求（可参考 5G-A 套餐 199 元资料改写）→ 提报单止于确认点 → 回复【发起需求审批】→ 输出执行方案表格 + req_id（节点结果存储保存，**生成后止于确认点**） |
| 幕2 用户确认 | 确认门禁有效性 | 先演示"执行方案未确认直接要求配置→拒绝"（方案就绪止于确认点，不调度 wf_sub_02），再回复"确认执行"→ 触发执行主干首环节（智能配置） |
| 幕3 执行主干步进推进 | 步进式推进纪律 + 每环节结果打印 | 回复"确认执行"后逐环节推进：每环节（智能配置→稽核→资费→测试）处理完输出结果+**下一步建议**并止于反馈点，回复【下一步】/【继续】后才进入下一环节 |
| 幕3A 异常中断与引导 | 异常处置完整性 + 续跑正确性 | 注入稽核缺陷 → 中断并打印异常节点/原因/建议 → 演示【重新执行】续跑 |
...
3. **审批跟踪**：需求工单审批（approval-type=requirement，由 `wf_sub_11` 在用户确认后发起）与上线审批（approval-type=launch）推送后，用户可随时发送"查询审批进度"消息调用 `query_approval_status`（双轨）查询审批状态；审批驳回时数字员工同步驳回原因并引导修改后重新发起。
4. **迭代机制**：
   - 知识库持续更新（新规范、新资费政策）；
   - 提示词与选择器阈值按稽核/测试误判情况调优；
   - 后端适配端点与工作流代码节点（`knowledge/`、`gen_workflows_v2.py`、12 子工作流 JSON）按系统升级同步维护。
...
| 用户未确认即触发生产写入 | 智能体提示词【技能1/2】+【限制2/3/5】约束：需求分析生成《加载方案》后**止于确认点**（未回复确认不调度 `wf_sub_02`），确认语义识别命中后才进入执行主干（后端 save_product_config 已移除 CONFIRMED 门禁，"未确认不配置"由智能体语义识别保证） |
| 执行主干中断后续跑重复执行已完成写操作 | 各子工作流以 req_id（统一键）、node_name=config/spec/fee/test/report 落库节点结果存储（后端持久化）；续跑时子工作流自查回放已成功环节，写接口不重复调用 |
| 未经确认发起审批 | **需求工单审批**：`wf_sub_00` 提报单就绪即止于确认点（不直接发起），须用户明确回复【发起需求审批】后才由 `wf_sub_11` 发起（智能体提示词【限制4】+ 3.2 意图映射表约束）；**上线审批**：后端 submit_release_approval 硬校验 req_id 四环节（config/spec/fee/test）结果齐全 + 智能体提示词【技能3】【限制8】审批引导约束 |
...
| 流程环节被跳过 | 双保险：智能体提示词步进式推进纪律（【限制5/6】严禁跳过环节、严禁并行调用、严禁凭语义推断成败、**每环节必须得到用户反馈确认后才推进下一环节，严禁未经确认擅自推进**）+ submit_release_approval 四环节结果硬校验（审批环节跳步即被拒绝）；配置落地环节依赖确认语义识别；需求工单审批依赖提报单确认点约束（未回复【发起需求审批】不调度 wf_sub_11） |

---

## 附：快速实施 Checklist

### A. 工作流 JSON 路线（V2.1 重塑后当前基线：12 子工作流 JSON + 智能体语义调度 + knowledge/ + 后端适配端点）
...
- [x] `gen_workflows_v2.py` 生成 **12 个子工作流 JSON**（`wf_sub_00`~`wf_sub_11`，无 `wf_main_intent` 意图调度主流程，见 `工作流配置/智能体工作流集V1.6/`）并导入
...
- [x] 智能体按 3.2 提示词【意图→工作流映射表】语义识别直调（需求提报/确认配置/失败续跑/上线审批/审批进度/监控运维/存量查询/存量合规/QNA 等），只做语义识别、业务理解与结果表达；确认门禁与结果表达由【限制】约束，req_id 透传/环节落库/成败判定/续跑回放/审批硬校验由子流自查链路+代码节点+后端硬校验兜底；执行主干逐环节推进、每环节处理完输出结果+**下一步建议**并止于反馈点（V3.6 步进式人机协同）
- [x] 环节覆盖：需求提报（wf_sub_00 渲染提报单止于确认点 + wf_sub_11 用户确认后发起需求工单审批 approval-type=requirement）、需求分析（wf_sub_01 模板轨）、智能配置（wf_sub_02 融合成员回显）、规格稽核（wf_sub_03 组维度）、资费校准（wf_sub_05 成员分组）、自动测试（wf_sub_04 31 条固定用例+九章节报告）、上线审批（wf_sub_06 双轨+自动上线+监控方案）、监控运维（wf_sub_07 根因闭环）、审批进度查询（wf_sub_08 双轨）、存量查询（wf_sub_09 只读）、存量合规（wf_sub_10）
...
- [ ] 端到端联调：正向全流程 + 反向用例（**执行方案未确认不配置**、需求审批未确认不发起、未确认不配置、跳步审批被拒、**执行主干逐环节推进（每环节处理完输出结果+下一步建议并止于反馈点，未获用户反馈确认不推进下一环节）**、稽核驳回中断引导、续跑不重复写）+ 18 销售品兼容回归（对齐 8.1/8.2 节）
- [ ] 全流程各工作流（需求提报/需求分析/智能配置/规格稽核/资费校准/自动测试/上线审批/监控运维及辅助子流）处理完成后均输出【下一步建议】引导，导航步进进度与建议动作一致（V3.6）
...
- [ ] 正反向用例全流程调试通过（重点验证：**执行方案确认门禁（方案生成止于确认点）**、未确认不配置（智能体确认语义识别）、**需求工单审批须确认后发起**、执行主干串行不停顿、每环节结果打印、异常中断与重新执行/修改执行方案引导、审批发起门禁、审批进度/监控结果消息查询、节点结果存储读写一致、实时稽核同步返回、测试报告含受理验证结论、wf_sub_02~05 提取原文代码节点取值正确）
- [ ] 按《端到端演示剧本》完成全流程彩排（含反向分支）
- [ ] 发布上线并接入监控运维
