...
- **取值链与本体推理引擎（V2.1，字段本体由后端推理引擎统一保证）**：
  ...
  逐字段执行：本体校验→非法值修正回写（月付/包月→按月等枚举归一）→缺失字段按
  本体默认值补全→兜底口径字段（套餐固定费/三类资源）置"待补充"不默认补全；
  方案输出拆分节点以推理后 fields_json 为准组装 plan_json（引擎兜底闭环，
  无法自动修正项经 violations 在结束节点提示用户）。
- **待补充触发条件（V1.9，仅两种，满足其一才判"待补充"）**：
  ① 套餐固定费（月租费）未提取到；
  ② 流量/语音/短信三类资源一个都未提取到（此时未提取到的资源字段全部填
  "待补充"；任一类已提取到则其余资源字段不算缺失，可AI补全）。
  待补充字段禁止推理，不得从相似产品照搬；提示用户在确认执行方案时补充。
