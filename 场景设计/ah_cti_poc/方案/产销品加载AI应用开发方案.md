...
> 版本：V2.7　日期：2026-09-14
> 实现基线（V2.6 起）：**Skills 技能包**（`skills/cpcp-product-worker/`）；第 2~6 章平台工作流描述为业务口径与契约权威定义（见 1.4 节实现方式说明）

> V2.5 履历 | 2026-09-13 | offerInfo 与需求要素统一配置结构模板规范（同构键值合并）已按 git 历史（9f84248）补录版本记录表第 19 行 |
| V2.6 | 2026-09-14 | **实现方式新增 Skills 技能包路线（当前实现基线）**：① 本应用已按《skills/Skills技能包实现方案.md》整体迁移为 **Skills 技能包**实现——原「8 个子工作流 wf_sub_01~08 + 14 个插件 + 3 类代码节点 + 平台知识库」分别迁移为**单一技能包 `cpcp-product-worker`** 内的 **4 份流程文档（references/flow-A~D，程序 A~D）+ `scripts/cpcp_api.py` 17 子命令统一客户端 + `references/` 参考文档（含 K1~K5 知识库 26 份）**，主 SKILL.md 承担原智能体常驻提示词（角色/意图路由/核心纪律）；② **业务口径零改动**：四类18字段、待补充判定（唯一维持待补充=套餐固定费）、取值链（原始需求→相似产品→本体默认值→待补充）、来源四态（原始需求/AI推理/本体推理）、req_id 统一键（PLAN+时间戳+3位随机）、异常矩阵 E1~E23、后端硬校验（5002/5006/四环节门禁/幂等）全部原样保留；③ **契约零改动**：脚本子命令与 2.1 节 14 工具契约一一对应（工具1 offerInfo 单对象+同构 fields 数组为 V2.5 口径），裸报文请求/超时重试/错误码归一下沉到脚本层；④ 8 个子工作流、平台插件与知识库配置**视为平台侧历史实现存档**（工作流/插件 JSON 已从工作区移除，可依 git 历史回溯），后续演进以 Skills 技能包为准；⑤ 6.2 节与 6.5 节同步；⑥ **结构精简**（同日）：8 份逐工作流环节程序（P1~P8）合并为 4 份流程文档（程序A=需求分析、程序B=执行主干四环节串行、程序C=上线审批、程序D=查询运维），枚举/命名/模板并入 ontology-fields.md（删除独立的 enums-and-naming.md 与 plan-templates.md），SKILL.md 精简为约 50 行（意图路由+核心纪律+目录导航） |
...
| V1.4 | 2026-09-12 | 明确待补充规则：仅**价格、资源**两类字段未提供时填"待补充"（禁止推理，由用户在确认环节补充）；**其余缺失字段一律由 LLM 基于相似产品（query_similar_offer 结果 + 存量销售品资料）推理补全**，来源标记"AI推理" |
...
| V2.3 | 2026-09-13 | **需求分析工作流重构为 7 环节新链路 + 缺值来源口径统一「AI推理」**：① wf_sub_01 按 7 环节重构——环节1 需求理解与要素拆解（LLM 职责收窄为**仅提取要素信息**：18字段未提及项 value 填空字符串、不做完整性判断、不输出 pending_fields，elements_json 增补为固定18项）；环节2 相似产品查询（入参=节点2需求要素摘要 need_summary，整合四类要素关键信息）；环节3 **产品信息整合**（原「字段映射与补全」节点重写职责：将匹配产品（相似度第1）的全量配置信息与要素部分信息整合，输出最终产品信息=18字段完整取值，source 仅"原始需求/AI推理"两种）；环节4 字段本体推理（工具14 reason 不变）；方案输出拆分/待补充项判断/保存/双结束节点保留；② **缺值来源全局口径统一**："AI推理"全部改称"AI推理"（节点2/4 提示词、插件描述、K1/K5 知识库、方案文档、演示剧本），字段存储值不变、兼容存量数据；③ 节点4 新增整合规则三级取值（需求要素有值→原始需求；无值→相似产品取值标 AI推理；两者皆缺失→留空由引擎补全标 AI推理）；6.1 表与节点表同步 |
...
| V2.5 | 2026-09-13 | **offerInfo 与需求要素统一配置结构模板规范（同构键值合并）**：① 后端 OfferSeedService 新增 toFields18——销售品种子（20键英文结构）→ 与需求要素解析同构的 fields 四类18字段数组（field/category/value，字段名与本体注册表一致：产品名称取 offer_name、套餐固定费=monthly_fee+元/月、流量/语音取 in_fee 中文键、副卡规则按 sub_card 允许办理+共享规则拼装、渠道类型顿号拼接 sale_channels 等），工具1 offerInfo 出参改为该同构结构（保留 similarOfferId/similarOfferName/series/sub_type 溯源键）；② 环节3 产品信息整合提示词改写为**同构键值合并**——两侧同一套配置结构模板规范，按 field 名逐字段对齐（需求有值→原始需求；无值→offerInfo 同名字段标 AI推理；皆缺失→留空由引擎补全），消除跨结构语义对齐；③ gen_plugins 工具1 出参 schema 补 offerInfo.fields 数组结构；节点3/4 提示词与 flowRemark 同步；双校验通过、后端编译通过 |
...
3. **需求分析结构化优先**：需求分析环节遵循《需求分析工作流可参考提示词.txt》方法论——业务要素→配置字段映射、字段来源仅"原始需求/AI推理"两种。**补全策略（V1.9 修订）：仅当套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到时填"待补充"（禁止推理，由用户在确认环节补充；任一类资源已提取到则其余资源字段可补全）；产品编码不做补全（由智能配置环节落地后生成）；其余缺失字段由大模型基于相似产品（query_similar_offer 结果 + 知识库存量销售品资料）推理补全，标记"AI推理"**，产出可执行的《产销品加载执行方案》。
...
1. 需求提报与分析：接收用户上传的需求文档或口述需求，按 6 步执行——
   ...
   识别信息完整性（原始需求/AI推理/待补充）、匹配历史相似产品
   （调用相似度分析工具 query_similar_offer）、基于历史产品补全字段、
   生成《产销品加载执行方案》（Markdown 表格 + 执行方案 JSON，经
   节点结果存储查询插件保存，返回存储 key/req_id）。
   需求原文是最高优先级，不得虚构。补全规则（V1.9）：
   仅当套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到时
   填"待补充"（禁止推理）；产品编码不补全（由智能配置生成）；
   其他缺失字段一律基于相似产品推理补全，来源标记"AI推理"。
   存在待补充字段时，在分析结果中明确列出并提示用户补充。
 2. 执行方案确认与执行主干触发（V2.2 调整：确认语义由智能体识别，后端不再校验确认标记）：
    向用户展示执行方案表格，等待用户确认。识别到用户确认类回复
    （"确认执行/同意/可以/没问题/执行吧"等）时：
    ① 取执行方案存储键 req_id（由 wf_sub_01 拆分代码节点以系统时钟生成，PLAN+yyyyMMddHHmmss+3位随机数；多轮上下文沿用原值，
...
4. 执行结果汇总与审批引导（V1.7）：四个环节全部成功后，打印各环节执行成功
...
| 首次提报需求/修改需求重新分析 | 调用 wf_sub_01 需求分析 | requirement_text=用户需求（含修改意见） |
 | 确认执行（确认执行/同意/可以/没问题/执行吧） | 串行调 wf_sub_02→wf_sub_03→wf_sub_05→wf_sub_04（V2.2 起无需写 CONFIRMED 确认标记，确认语义由智能体识别） | req_id=<上一次 req_id>（执行方案存储键，沿用原值）；后续环节入参取上一环节出参 |
...
2. 智能配置（配置落地）必须由用户确认执行方案后触发；执行方案 JSON 以
   ...
   识别"确认执行"类回复后直接按【意图→子工作流智能调度映射表】串行直调子工作流
...
3. 字段来源只允许"原始需求"或"AI推理"两种，禁止出现"系统默认/推测/猜测"；
   AI推理不得标记为原始需求。"待补充"仅两种触发情形：套餐固定费（月租费）未提取到，或流量/语音/短信三类资源全部未提取到；产品编码不补全（由智能配置生成）。
...
**① 需求分析大模型节点（需求分析助手，V1.2 重构；V2.3 重构为 7 环节新链路）**
- 温度值：0.2
- 输入：`requirement_text`（+ `query_similar_offer` 返回的相似产品列表）
- 链路（V2.3）：**环节1 需求理解与要素拆解**（仅提取要素：18字段未提及项 value 填空字符串，不做完整性判断、不输出 pending_fields）→ **环节2 相似产品查询**（入参=要素摘要 need_summary）→ **环节3 产品信息整合**（匹配产品全量配置 + 要素部分信息 → 最终产品信息，source 仅"原始需求/AI推理"）→ **环节4 字段本体推理**（工具14 reason 一体推理）
- 补全规则（V1.9 修订）：**套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到 → "待补充"（禁止推理；任一类资源已提取到则其余资源字段可补全）**；产品编码 → 禁止补全，值填"由智能配置生成"；其余缺失字段 → 基于相似产品推理补全，来源"AI推理"
...
```json
{
  ...
  "fields": [
    ...
    {"分类": "营销资源", "字段": "收费方式", "值": "月付", "来源": "AI推理"},
    {"分类": "销售规则", "字段": "退订生效规则", "值": "次月生效", "来源": "AI推理"}
  ],
  "similar_offers": [{"similarOfferId": "900102308", "similarityScore": "0.92"}],
  "pending_fields": ["流量资源", "套餐固定费"]
}
```
- 补全判定口径（V1.9）：套餐固定费（月租费）未提取到，或流量/语音/短信三类资源一个都未提取到 → 未提取到的字段"待补充"（任一类资源已提取到则其余资源字段可补全）；产品编码 → 值填"由智能配置生成"（禁止补全）；收费方式、生效规则、退订规则、渠道类型、适用地区、订购/退订限制等 → 基于最高相似度产品对应字段值推理，标记"AI推理"。
...
| AI推理字段标记正确率 | 100%（来源仅"原始需求/AI推理"两种，无违规来源） |
| 待补充规则符合率 | 100%（仅套餐固定费未提取到或三类资源全部未提取到时填"待补充"，其余字段无"待补充"残留且均有 AI推理值；产品编码均为"由智能配置生成"） |
