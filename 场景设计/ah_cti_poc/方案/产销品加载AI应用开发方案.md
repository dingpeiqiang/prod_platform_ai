...
| V2.0 | 2026-09-13 | **字段补全稳定性增强（本体+相似产品组合补全）**：新增知识分类 **K6 字段本体库**（`K6本体_产销品字段本体_V1.0.md`，四类18字段的类型/枚举/格式/默认规则/兜底口径定义）；wf_sub_01 节点2/节点4 提示词升级为**两阶段补全**——先取 K6 本体约束字段形态（枚举归一：月付/包月→按月；销售品状态新需求一律"待上线"；数值带单位等），再按**逐级降级取值链**（原始需求 → 相似产品 → 本体默认值 → 待补充）取值，禁止跳级虚构；知识库分类由 5 个扩展为 6 个（挂载点：wf_sub_01 节点2/节点4） |
...
| `knowledge\` 目录 27 份文档（见 `knowledge\README_知识库总索引.md`） | 知识库冷启动语料，K1~K6 六个分类：K1 业务规范 3 份 / K2 资费规则 2 份 / K3 测试规范 2 份 / K4 存量销售品资料 18 份（18 销售品逐个拆分切片） / K5 FAQ 1 份 / K6 字段本体 1 份（V2.0 新增：四类18字段类型/枚举/默认规则定义，补全稳定性锚点）（试用版） |
...
# 信息完整性与来源规则
字段三态：原始需求 / 可AI补全 / 待补充。
- **组合补全策略（V2.0，本体+相似产品，消除 LLM 自由推理的不稳定）**：
  逐级降级取值：**原始需求 → 相似产品（最高相似度） → K6本体默认值 → 待补充**，
  禁止跳级虚构。字段形态必须符合 K6《产销品字段本体》约束：
...
- **除上述待补充情形外的其他缺失字段，按组合补全策略取值**：以
  query_similar_offer 返回的相似产品（及知识库存量销售品资料）中该字段的
  稳定取值为依据（须符合 K6 本体枚举/格式），相似产品亦缺失时按 K6 本体
  默认值；来源统一填"AI补全"。
