...
> 依据：《产销品加载AI应用开发方案.md》V2.9（1.4 节实现方式说明、3.1/3.2/3.3 节）、《产销品加载AI应用-细化设计方案.md》V2.2（第 5 章部署清单）、《skills/Skills技能包实现方案.md》V1.3
> 用途：数字员工创建表单填写 + 技能包部署与后端就绪逐项核对（V2.1 新增第 1 节"数字员工创建表单"，对应平台创建助手界面逐字段填写值；V2.2 字段体系重构为 3 模块/9 分类/24 字段，输出样例对齐《加载方案》五列模块表格，来源标注两态【原始需求】/【AI补全】）
...
| 助手描述/功能介绍 | 面向产销品域的数字员工，支持从需求提报、需求分析（五列模块表格《加载方案》）、用户确认、智能配置（配置落地）、配置规格稽核、资费校准、自动测试、受理验证到上线审批（上线校验看板）、监控运维方案的全流程自动化操作。 | 开发方案 3.1 节（技能触发描述可用 SKILL.md description：当用户提出销售品需求提报、执行方案确认、执行主干触发、上线审批发起、审批进度/运行监控查询或产销品业务问答时使用） |
...
| 执行主干调度 | 加载 flow-B（智能配置→配置规格稽核→资费校准→自动测试→受理验证 串行），每环节打印结果，异常中断引导 | 确认配置 / 确认执行 / 执行稽核 / 资费校准 / 自动测试 / 受理验证 / 重新执行失败环节 |
| 上线审批 | 加载 flow-C（程序C）：自查四环节结果→上线校验看板（5 项 ✅ 表）→submit_approval 推送→输出审批单号 | 上线审批 / 发起审批 |
...
2. 执行主干 智能配置→稽核→资费校准→自动测试→受理验证 必须串行：严禁并行、跳过环节、重复调用已成功环节；失败立即中断引导，不得自行重试或跳过；
...
| 技能描述 | 安徽电信 CPCP 产销品域数字员工技能。当用户提出销售品需求提报、执行方案确认（确认配置）、智能配置、配置规格稽核、资费校准、自动测试、受理验证、上线审批、监控运维查询或产销品业务问答时使用；支撑销售品从需求到上线的端到端自动化加载（需求分析→智能配置→稽核→资费校准→自动测试→受理验证→上线审批→监控运维）。 | SKILL.md frontmatter description |
...
| 表单字段 | 填写值 | 来源 |
| --- | --- | --- |
| 开场白 | 您好，我是产销品数字员工，可以帮您完成销售品从需求提报、加载方案生成、智能配置、规格稽核、资费校准、自动测试、受理验证到上线审批的全流程操作。您可以直接描述需求，或发送：- 查询审批进度（需审批单号或销售品ID）- 查询销售品监控结果（需销售品ID）- 重新执行上次失败环节 | SKILL.md 开场白（开发方案 3.3 节为扩展口径， Skills 版以 SKILL.md 为准） |
...
| Q12 | 数字员工支持什么，不支持什么？ | 支持：需求分析与加载方案生成（五列模块表格）、确认配置后自动执行（智能配置→稽核→资费校准→自动测试→受理验证）、上线校验看板与审批发起、监控运维方案与运行监控查询、存量问答。不支持范围外问题（如普通客服咨询），返回拒答话术 |

### 1.6 表单填写核对要点

- [ ] 助手 ID/名称/描述与 1.1 一致；发布范围"所有人可见"；
- [ ] 角色与限制逐字引用 1.2（核心纪律 5 条 + 拒答话术不得删改）；
- [ ] 能力项与意图路由 5 类一一对应（执行主干含受理验证共 5 环节；超范围问题命中拒答话术）；
- [ ] 技能包挂载后 `SKILL.md` 入口可达，环境变量 `CPCP_BASE_URL` 已配置；
- [ ] 开场白与 3 条引导问题与 1.4 逐字一致；
- [ ] FAQ 12 条录入完成，直接返回=否；抽查 Q2/Q10 回答命中 K4/K5 对应描述；Q3/Q6/Q12 为 V3.0 口径（套餐档位/AI补全、双重门禁、受理验证+监控运维方案）。
