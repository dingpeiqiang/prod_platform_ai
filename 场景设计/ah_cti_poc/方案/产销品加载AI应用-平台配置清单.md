...
- [ ] 工具2/7/9 关键校验点（V1.7 后端硬校验）：工具2 同步返回无文件上传；工具7 校验存储中 req_id 的 **CONFIRMED 确认标记**（无标记返回 NOT_CONFIRMED，V2.1 起返回体附 `save_result.diagnose` 字段列出该 req_id 已有环节，区分"LLM 跳步未写确认标记"与"req_id 传错"）；工具9 校验 **req_id 四环节（config/spec/fee/test）结果齐全**（缺失即拒绝推送）
