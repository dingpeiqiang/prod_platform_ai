...
| V1.5 | 2026-09-13 | 对齐主方案 V2.1（**字段补全改为本体推理引擎实现**）：① 删除 K6 字段本体库，知识分类 6→5（恢复 K1~K5）；② 3.4.2 组合补全策略改写为「取值链 + 后端字段本体推理引擎（工具14 field_ontology_reason，action=validate 校验/complete 默认值补全）」；③ wf_sub_01 新增「字段本体校验」节点（节点4 后旁路质量关卡）；④ 4.1/4.2/4.3/6.3 同步（K6 行删除、知识库挂载恢复 K1~K5） |
...
字段形态校验（wf_sub_01「字段本体校验」节点，工具14 action=validate 逐字段推理，违反即提示重填）：
...
其他分类字段缺失
    → 按取值链取值（相似产品同值优先，相似产品亦缺失时留空由本体推理引擎按默认值补全），来源="AI补全"
产品编码：恒填"由智能配置生成"（不计入 pending_fields）
来源枚举校验：仅允许 {原始需求, AI补全}，出现其他值 → 校验失败重新生成
枚举值校验：字段值不符合本体推理引擎枚举/格式 → violations 返回期望规则，提示重填（引擎锚点兜底）
```
