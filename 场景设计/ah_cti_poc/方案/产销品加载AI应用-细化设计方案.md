...
- **页面地址输出（单面板约定）**：每个子工作流结束节点统一在末尾输出 `{panel}`（由独立代码节点 type=6 `panel_code_node` 构建的 ```` ```xsbot-panel ```` 片段，单面板单 url、**JSON 紧凑无空格逐字对齐参考示例**）。url 用环节业务页 `config-workbench.html?offer_id=..&name=..&chatId=<实值>&stage=<N>&view=stage`，`title` 用环节业务名；`chatId` 运行期以 `chat_id` 实值填充。有 `offer_id` 的环节取 `offer_id`，需求/方案阶段（00/01）取 `req_id` 兜底。生成辅助与口径见《工作流JSON开发规范.md》§十一。
