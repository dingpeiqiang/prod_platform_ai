...
字段形态校验与默认值补全（wf_sub_01「字段本体推理」节点，工具14 action=reason 一体推理，串行闭环：校验→非法值修正回写→缺失默认值补全→兜底置待补充，方案以推理后结果为准）：
...
枚举值校验：字段值不符合本体推理引擎枚举/格式 → 引擎自动修正回写（枚举归一：月付/包月→按月等）；无法修正的经 violations 在结束节点提示用户（引擎兜底闭环）
```
