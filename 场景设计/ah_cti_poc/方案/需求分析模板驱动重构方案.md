# cpcp-product-worker 需求分析链路重构方案（模板驱动六步流程）V1.1

> 版本：V1.1（6 项评审结论已确认，见第 6 章；S0 完成） | 日期：2026-09-16
...
### 1.1 总体链路（新 flow-A' 模板驱动）

```
需求原文
 → ①LLM 产品识别（产品列表[]，每项 product_type→templateId 路由；封闭 schema）
 → ②逐产品相似查询（本地存量目录检索 offer_id + 实例化报文；远端 similar_offer 兜底）
 → ③模板获取（templates/_index.json 路由 → <templateId>.schema.json）
 → ④LLM 逐产品按模板提取（提示词注入该模板 schema 骨架；输出嵌套 elements，仅命中路径）
 → ⑤merge_nested 逐产品合并（elements + 相似品实例化报文，JSONPath 对位）
 → ⑥render_table 逐产品渲染层级表格（模板树 L1~Ln 分组）
...
skills/cpcp-product-worker/
  scripts/
    cpcp_api.py            增子命令：identify_products(①)/similar_local(②)/get_template(③)/merge_nested(⑤)/render_table(⑥)
                           （既有 merge_fields 等零改动；新子命令独立函数，复用 _read_arg/_err/_http）
    derive_flat24.py       新增：plan_json_v2 → 24 字段 plan_json 确定性投影
    templates/             从 方案/templates/ 迁入（6 schema + _index.json）
  references/
    flow-A-requirement.md  重写为双轨程序 A（旧步骤1~8 保留 + 新步骤 A1~A8 模板轨）
    ontology-fields.md     保留（旧轨 + derive_flat24 映射依据）
    templates-registry.md  新增：6 模板注册表（templateId/适用产品类型/leafs 数/价格字段表/x-show-when 汇总）
...
- **质量自检（代码化，防提取失败）**：`validate_elements.py` 新增——路径合法性（∈ schema）/数值合法性/枚举命中率统计；命中率 < 阈值（P0 定 60%）→ 提示词重试一次，再低中断人工。
- **dup 与产品信息.txt 纪律**：需求与存量品同名的只标注；900102306→900113046 指向 active。

### 2.5 第⑤步 合并（merge_nested V1.0 已完成，增量小）

- 增量 1：`--mode legacy`（存量实例化：source 标签"存量提取"+ 价格豁免）——已在存量实例化方案中定义，同一脚本落地；
- 增量 2：出参增加 `plan_md_v2`（直接内联调用 render_table）或保持双命令分离（**建议分离**，工具职责单一，由 flow 文档串联两次调用）；
- 增量 3：enum 校验（提取值 ∉ enum → 出参 `_meta[path].enum_violation=true`，不阻断，进异常清单）。

### 2.6 第⑥步 渲染（render_table V1.1 已完成，增量小）
...
- 映射表：模板路径 → 24 字段名，固化在 `references/templates-registry.md` 附录（如 `optionalInfo.printContent.prcMonthFee`→套餐档位+计费口径；`baseInfo.roleMax`→无对应丢弃；24 字段无对应路径的填默认值口径同 ontology-fields.md）；
...
- 落位：**skill 侧** `references/K5存量报文/`（第②步本地相似检索的读取路径，与 K4存量 md 并列；方案目录留生成工具与中间产物）；
- dup（900102306）：复制 active 版加 `x-dup-of` 标记。

### 3.2 模板注册表（新增 references/templates-registry.md）

- 内容：6 模板 × {templateId, name_cn, product_type, leafs, 价格字段路径表, x-show-when 计数, enum 总数, flat24 映射表}；
...
| 文件 | 改动 |
| --- | --- |
| `flow-A-requirement.md` | **整体改写为模板轨**（步骤 A1~A8 替换步骤 1~8；旧轨内容迁 `方案/_deprecated/flow-A-v61-备份.md` 存档）；取值链改为：原始需求 → 相似品模板报文（AI补全）→ schema default（默认值）→ 待补充（价格类） |
| `tools-contract.md` | 增工具20~24（identify_products/get_template/merge_nested/render_table/derive_flat24）契约，格式对齐工具19；工具1 similar_offer 契约登记 `offerTemplate` 出参（后端 POC 改造项）；工具19 merge_fields 标 @deprecated |
| `SKILL.md` | 入口调度表 intent=REQ_REPORT 注"flow-A 模板轨（V7.0）"；目录导航加 templates-registry.md 与 K5存量报文；核心纪律增"模板轨价格纪律与合并纪律"两条（与纪律9/10 同款表述）；开场白不变 |
| `exception-matrix.md` | 增 E30（模板路由失败：LLM product_type ∉ 枚举）/E31（提取质量门禁未达标：枚举命中率/必填缺失率超阈）/E32（schema 文件缺失/损坏）；复用 E1 中断询问形态 |
| `ontology-fields.md` | 标注"flat24 派生映射依据（下游过渡兼容）"，不再作为提取注册表 |
...
| 6 | 存量报文落位 | **K5（skill 内）** | `references/K5存量报文/` 18 份实例化报文 + `_report.json`；第②步本地相似检索读取路径与此一致 |
