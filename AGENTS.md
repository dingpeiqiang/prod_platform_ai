...
| 步骤 | 操作 | 说明 |
| ---- | ---- | ---- |
| 1 | `git status` | 确认当前分支与工作区变更状态 |
| 2 | 检查 `.gitignore` | 确认构建产物、临时文件等已正确排除 |
| 3 | `git diff` | 查看具体变更内容 |
| 4 | 确认提交范围 | 与用户确认本次要提交的文件集合 |
...
# 2. 选择性添加变更文件（优先使用精确路径而非 git add .）
git add <file1> <file2> ...
...
- **禁止直接 `git add .`**：避免误提交敏感文件或构建产物
- **pre-commit 钩子失败处理**：若触发自动 stash，执行 `git stash list` 并按提示恢复后再继续
- **推送前确认**：必须展示提交摘要给用户确认后再 push
- **分支检查**：确认当前分支正确，避免误推送到错误分支
