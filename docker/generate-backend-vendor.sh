...
# ============================================================
# AI驱动动态表单 - 后端依赖包生成脚本
# 使用方式: bash generate-backend-vendor.sh [--docker|--venv]
# 说明: 下载所有依赖包到 vendor 目录
#       --docker: 使用 Docker 容器生成（推荐，环境更干净）
#       --venv:   使用临时虚拟环境生成（解决环境问题）
# ============================================================
...
TEMP_VENV="/tmp/prod_platform_venv"
...
if [ "$USE_DOCKER" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo ""
        echo "错误: Docker 下载失败"
        echo "建议尝试: bash generate-backend-vendor.sh --venv"
        exit 1
    fi
elif [ "$USE_VENV" = true ]; then
    # 使用临时虚拟环境下载依赖
    echo "使用临时虚拟环境下载依赖..."
    echo "虚拟环境目录: $TEMP_VENV"
    echo ""
    ...
    source "$TEMP_VENV/bin/activate" && \
        ...
        pip download --no-cache-dir -r "$REQ_FILE" -d "$VENDOR_DIR"
    ...
    if ! command -v pip >/dev/null 2>&1; then
        ...
        echo "或者使用: bash generate-backend-vendor.sh --docker 或 --venv"
        exit 1
    fi
    ...
    if [ $? -ne 0 ]; then
        echo ""
        echo "错误: 下载失败"
        echo "建议使用: bash generate-backend-vendor.sh --docker 或 --venv"
        exit 1
    fi
fi
